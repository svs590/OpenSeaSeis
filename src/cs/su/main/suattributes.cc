...
void* main_suattributes( void* args )
{
	...
	su2cs->setSUDoc( sdoc_suattributes );
	...
	if (!parObj.getparint("trend", &trend))	trend=0;
	...
	case FREQW:
		if (!parObj.getparfloat("unwrap", &unwrap))	unwrap=1;
		if (!parObj.getparint("wint", &wint))	wint=3; 
	break;
	case THIN:
		if (!parObj.getparfloat("unwrap", &unwrap))	unwrap=1;
		if (!parObj.getparint("wint", &wint))	wint=3;
	break;
	case ARG:
		if (!parObj.getparfloat("unwrap", &unwrap))	unwrap=0;
	break;
	}
	...
	do {
		...
		break;
		default:
			throw cseis_geolib::csException("%s: mysterious mode=\"%s\"", __LINE__, mode);
		}
