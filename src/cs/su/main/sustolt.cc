...
void* main_sustolt( void* args )
{
	...
	ft = tr.delrt/1000.0;
	if (ft!=0.0) throw cseis_geolib::csException("cannot handle non-zero time of first sample");
	offset = tr.offset;
	...
	if (!parObj.getparint("noffmix",&noffmix)) noffmix = 1;

	/* get times and velocities */
	/* ...times */
	ntmig = parObj.countparval("tmig") + 1; 
	tmig = ealloc1float(ntmig);
	if (!parObj.getparfloat("tmig",tmig)) tmig[0] = 0.0;
	tmig[ntmig - 1] = (nt*dt + ft);  /* set last time to end of data */
	
	/* ... velocities */
	nvmig = parObj.countparval("vmig") + 1 ;
	if (nvmig!=ntmig) throw cseis_geolib::csException("number of tmig and vmig must be equal");
	vmig = ealloc1float(nvmig);
	if (!parObj.getparfloat("vmig",vmig)) vmig[0] = 1500.0;

	vmig[nvmig -1 ] = vmig[nvmig - 2]; /* set velocity at end of data to */
					   /* last velocity parObj.getparred */
	...
	if (!parObj.getparfloat("smig",&smig)) smig = 1.0;
