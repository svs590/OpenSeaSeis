...
void* main_sugassmann( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sugassmann );
	...
	/* get parameters */
	nt = 500;	parObj.getparint("nt", &nt);
	CHECK_NT("nt",nt);				tr.ns = nt;
	ntr = 200;	parObj.getparint("ntr", &ntr);
	dt = 0.004;	parObj.getparfloat("dt", &dt);		tr.dt = dt*1000000;
	mode = 0;	parObj.getparint("mode", &mode);
	if (mode != 0 && mode != 1) mode = 0;
	it1 = nt/2;	parObj.getparint("it1", &it1);

	/* environment vars */
	temp = 140;		parObj.getparfloat("temp", &temp);
	pres = 200;		parObj.getparfloat("pres", &pres);
	/* caprock vars */
	v1 = 3300;		parObj.getparfloat("v1", &v1);
	r1 = 2.6;		parObj.getparfloat("r1", &r1);
	/* fluid vars */
	g = 0.56;		parObj.getparfloat("g", &g);
	api = 50;		parObj.getparfloat("api", &api);
	s = .35;		parObj.getparfloat("s", &s);
	so = .7;		parObj.getparfloat("so", &so);
	sg = .2;		parObj.getparfloat("sg", &sg);
	/* rock frame vars */
	kmin = 37900;	parObj.getparfloat("kmin", &kmin);
	mumin = 44300;	parObj.getparfloat("mumin", &mumin);
	rmin = 2.67;	parObj.getparfloat("rmin", &rmin);
	phi = 0.24;		parObj.getparfloat("phi", &phi);
	a = 1;			parObj.getparfloat("a", &a);
	b = 15;			parObj.getparfloat("b", &b);
	c = 1;			parObj.getparfloat("c", &c);
	h = 20;			parObj.getparfloat("h", &h);
	/* mode opertation vars */
	p = .15;		parObj.getparfloat("p", &p);
...
*******************************************************************/
{
	...
	if ( so + sg > 1 ) throw cseis_geolib::csException("so + sg > 1...impossible");
