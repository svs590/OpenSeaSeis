...
void* main_suvelan( void* args )
{
	...
	if (!parObj.getparfloat("anis1",&anis1)) anis1 = 0.0;
	if (!parObj.getparfloat("anis2",&anis2)) anis2 = 0.0;
		...
		if (gottrace) {
			...
			cdp = tr.cdp;
                        if ((1.0 + offset*offset*anis2) <= 0.0)
                           throw cseis_geolib::csException("anis2 too small");
