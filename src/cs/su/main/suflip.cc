...
void* main_suflip( void* args )
{
	...
	if (!parObj.getparint("flip",    &flip))    	flip = 1;
	if (flip != -1 && flip != 0 && flip != 1 && flip != 2 && flip != 3)
		throw cseis_geolib::csException("flip = %d, flag must be -1, 0, 1, 2, or 3", flip);
		...
		register int ncol, register int nrow, int flip)
{
	...
	default:	/*  defensive programming  */
		...
		throw cseis_geolib::csException("%d: mysterious flip value: %d", __LINE__, flip);
	}
	return;
}
