...
void* main_sudmofkcw( void* args )
{
	...
	if (!parObj.getparfloat("gamma",&gamma)) gamma = 0.5;
	if (!parObj.getparint("ntable",&ntable)) ntable = 1000;
	if (!parObj.getparfloat("sdmo",&sdmo)) sdmo = 1.0;
	if (!parObj.getparint("flip",&flip)) flip=0;
	if (flip)
		sign = -1.0;
	else
		sign = 1.0;
...
*****************************************************************************/
{
	...
	for(i=0,b=bstart; i<ntable; i++,b+=(db)){
		...
		if(z<0) throw cseis_geolib::csException("negative sqrt when generating table");
		zoh[i] = sqrt(z);
		boh[i] = b;
	}
}
