...
void* main_setbhed( void* args )
{
	...
	if (!parObj.getparint("jobid", &jobid))	jobid = 0;
	if (!parObj.getparint("lino", &lino))		lino = 0;
	if (!parObj.getparint("reno", &reno))		reno = 0;
	if (!parObj.getparint("ntrpr", &ntrpr))	ntrpr = 0;
	if (!parObj.getparint("hdt", &hdt))		hdt = 0;
	if (!parObj.getparint("nart", &nart))		nart = 0;
	if (!parObj.getparint("dto", &dto))		dto = 0;
	if (!parObj.getparint("hns", &hns))		hns = 0;
	if (!parObj.getparint("nso", &nso))		nso = 0;
	if (!parObj.getparint("format", &format))	format = 0;
	if (!parObj.getparint("fold", &fold))		fold = 0;
	if (!parObj.getparint("tsort", &tsort))	tsort = 0;
	if (!parObj.getparint("vscode", &vscode))	vscode = 0;
	if (!parObj.getparint("hsfs", &hsfs))		hsfs = 0;
	if (!parObj.getparint("hsfe", &hsfe))		hsfe = 0;
	if (!parObj.getparint("hslen", &hslen))	hslen = 0;
	if (!parObj.getparint("hstyp", &hstyp))	hstyp = 0;
	if (!parObj.getparint("schn", &schn))		schn = 0;
	if (!parObj.getparint("hstas", &hstas))	hstas = 0;
	if (!parObj.getparint("hstae", &hstae))	hstae = 0;
	if (!parObj.getparint("htatyp", &htatyp))	htatyp = 0;
	if (!parObj.getparint("hcorr", &hcorr))	hcorr = 0;
	if (!parObj.getparint("bgrcv", &bgrcv))	bgrcv = 0;
	if (!parObj.getparint("rcvm", &rcvm))		rcvm = 0;
	if (!parObj.getparint("mfeet", &mfeet))	mfeet = 0;
	if (!parObj.getparint("polyt", &polyt))	polyt = 0;
	if (!parObj.getparint("vpol", &vpol))		vpol = 0;
        parObj.checkpars();
