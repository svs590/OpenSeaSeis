...
void* main_sudatumk2ds( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sudatumk2ds );
	...
	es = fs+(ns-1)*ds;
        ...
        if (!parObj.getparint("nxi",&nxi)) throw cseis_geolib::csException("must specify nxi!\n");
        if (!parObj.getparfloat("fxi",&fxi)) throw cseis_geolib::csException("must specify fxi!\n");
        if (!parObj.getparfloat("dxi",&dxi)) throw cseis_geolib::csException("must specify dxi!\n");
        if (!parObj.getparint("sgn",&sgn)) throw cseis_geolib::csException("must specify sgn!\n");

	/* optional parameters	*/
	if (!parObj.getparint("nxso",&nxso)) nxso = (nxt-1)*2+1;
	if (!parObj.getparfloat("fxso",&fxso)) fxso = fxt;
	if (!parObj.getparfloat("dxso",&dxso)) dxso = dxt*0.5;
        if (!parObj.getparint("nxgo",&nxgo)) nxgo = (nxt-1)*2+1;
        if (!parObj.getparfloat("fxgo",&fxgo)) fxgo = fxt;
        if (!parObj.getparfloat("dxgo",&dxgo)) dxgo = dxt*0.5;
        if (!parObj.getparfloat("exgo",&exgo)) exgo = fxgo + (nxgo-1)*dxgo;
        if (!parObj.getparint("antiali",&antiali)) antiali=1;
	...
	if (!parObj.getparfloat("offmax",&offmax)) offmax = 999999;
...
**************************************************************************/
{
        ...
        for (ir=0; ir<nr; ++ir) {
                ...
                if (!decodeSurface(t,sgn,&nxz[ir],&x[ir],&z[ir]))
                        throw cseis_geolib::csException("Surface number %d specified "
                                "incorrectly!\n",ir+1);
        }
...
*****************************************************************************/
{
	...
	for(ix=nxf; ix<=nxe; ++ix){
		...
		if(nzp<iz0) nzp = iz0;
                ...
                if(iz>=nz)
                        throw cseis_geolib::csException("Datuming surface is out of output range!\n");
                az = z0+iz*rdz;
                jz = (int)az;
