...
void* main_suinterp( void* args )
{
	...
	if (!parObj.getparint("lenx",&lenx)) lenx=1;
	if (!parObj.getparint("nxmax",&nxmax)) nxmax=500;
	if (!parObj.getparint("xopt",&xopt)) xopt=0;
	if (!parObj.getparint("ninterp",&ninterp)) ninterp=1;
	if (!parObj.getparint("iopt",&iopt)) iopt=0;
	if (!parObj.getparint("deriv",&deriv)) deriv=0;
	if (!parObj.getparint("linear",&linear)) linear=0;
