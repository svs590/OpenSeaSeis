...
void* main_sufdmod2_pml( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sufdmod2_pml );
	...
	if (!parObj.getparfloat("fz",&fz)) fz = 0.0;
        ...
        if (!parObj.getparfloat("pml_max",&pml_max)) pml_max = 1000.0;
        if (!parObj.getparint("pml_thick",&pml_thick)) pml_thick = 0;
	...
	if (*hsfile!='\0') {
		if((hseisfp=fopen(hsfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open hsfile=%s",hsfile);
	...
	if (*vsfile!='\0') {
		if((vseisfp=fopen(vsfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open vsfile=%s",vsfile);
	...
	if (*ssfile!='\0') {
		if((sseisfp=fopen(ssfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open ssfile=%s",ssfile);
	...
	if (*dfile!='\0') {
		if((densityfp=fopen(dfile,"r"))==NULL)
			throw cseis_geolib::csException("cannot open dfile=%s",dfile);
		if (fread(od[0],sizeof(float),nx*nz,densityfp)!=nx*nz)
			throw cseis_geolib::csException("error reading dfile=%s",dfile);
		fclose(densityfp);
		dmin = dmax = od[0][0];
		for (ix=0; ix<nx; ++ix) {
			for (iz=0; iz<nz; ++iz) {
