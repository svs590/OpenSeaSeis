...
void* main_sulprime( void* args )
{
	...
	/* get parameters */
	if (!parObj.getparfloat("b",&btarg)) 		btarg   = 2.0;
	if (!parObj.getparfloat("dz",&dz)) 		dz	= 1.0;
	if (!parObj.getparfloat("f",&freq)) 		freq	= 60.0;
	if (!parObj.getparint("nmax",&nmax))		nmax	= 301;
