...
void* main_susyncz( void* args )
{
	...
	if (!parObj.getparint("nline",&nline))	     nline=1;
	if (!parObj.getparint("ntr",&ntr))	     ntr=32;	 tr.ntr = ntr*nline;
	if (!parObj.getparfloat("dx",&dx))	     dx=10.0;
	if (!parObj.getparint("nt",&nt))	     nt=128;
	CHECK_NT("nt",nt);					 tr.ns = nt;
	if (!parObj.getparfloat("dt",&dt))	     dt=0.004;	 tr.dt = dt*1000000;
	if (!parObj.getparfloat("tdelay",&tdelay))  tdelay=0.0;
						tr.delrt = tdelay*1000;
	if (!parObj.getparint("ninf",&ninf))	ninf=4;
	...
	/* parObj.getpar the reflector intercepts, dips, v's, and rho's */
	zint[0] = 0.0; /* surface hard wired */
	if (!parObj.getparfloat("zint", zint+1))
 		for (i = 1; i <= ninf; ++i) zint[i] = 100.0 * i;
	dip[0] = 0.0; /* surface hard wired */
	if (!parObj.getparfloat("dip", dip+1))
 		for (i = 1; i <= ninf; ++i) dip[i] = 5.0 * i;
	if (!parObj.getparfloat("v", v))
 		for (i = 0; i <= ninf; ++i) v[i] = 1500.+ 500*i;
	if (!parObj.getparfloat("rho", rho))
 		for (i = 0; i <= ninf; ++i) rho[i] = 1.0;

	/* check dips and intercepts */
	for (i = 1; i <= ninf; ++i) {
		if (dip[i] < -90.0 || dip[i] > 90.0) throw cseis_geolib::csException("dips1");
		if (ABS(dip[i] - dip[i-1]) > 90.0) throw cseis_geolib::csException("dips2");
		dip[i] *= TORADS;
		
		if (zint[i] <= zint[i-1]) throw cseis_geolib::csException("intercept");
	}
	for (i = 0; i < ninf; ++i) {
		if (v[i] <= 0.0) throw cseis_geolib::csException("velocities");
		if (rho[i] <= 0.0) throw cseis_geolib::csException("density");
	}
	...
	for (j = 1; j < ninf; ++j) {
		...
		for (i = j-1; i > 0; --i) {
		    ...
		    if (ABS(temp) > 1.0) throw cseis_geolib::csException("specular %d", j);
	...
	for (j=0; j<ninf; ++j) {
		...
		for (j2=j+1; j2<=ninf; ++j2) {
			...
			if (intercept > xmin && intercept < xmax) {
				throw cseis_geolib::csException("intercept");
			}
		    }
		}
