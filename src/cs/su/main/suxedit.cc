...
void* main_suxedit( void* args )
{
	...
	if (argc > 2)  throw cseis_geolib::csException("only one filename argument is allowed");
	...
	if (!isatty(STDIN)) {	/* stdin was redirected to file */
		...
		/* Admit defeat */
			...
			throw cseis_geolib::csException("can't open %s for reading", argv[1]);
		}
	}
