...
void* main_segywrite( void* args )
{
	...
	if (trmin < 1 || trmax < 1 || trmax < trmin)
		throw cseis_geolib::csException("bad trmin/trmax values, trmin = %d, trmax = %d",
			trmin, trmax);
	...
	if (buff) {
		if (EBCBYTES != write(tapefd, ebcbuf, EBCBYTES)) {
			...
			if (++errcount > errmax)
				throw cseis_geolib::csException("exceeded maximum io errors");
	...
	ns = bh.hns;
	if (!ns) throw cseis_geolib::csException("bh.hns not set in binary header");
	...
	do {
		...
		if (itr >= trmin) {
			...
			if(FALSE == xdr_setpos(&trhd_xdr,trstart)) 
			    throw cseis_geolib::csException("%s: trouble \"seeking\" start of trace",
				__FILE__);
...
*/
	...
	} while (cs2su->getTrace(&tr) && itr < trmax);
