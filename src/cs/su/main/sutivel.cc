...
void* main_sutivel( void* args )
{
	...
	if (!parObj.getparfloat("a",&a)) a = 2500.;
	if (!parObj.getparfloat("b",&b)) b = 1250.;
	if (!parObj.getparfloat("e",&e)) e = .20;
	if (!parObj.getparfloat("d",&d)) d = .10;
	if (!parObj.getparfloat("maxangle",&maxangle)) maxangle=90.;
	if (!parObj.getparint("nangle",&nangle)) nangle = 9001;
	if (!parObj.getparint("np",&np)) np=8001;
	if (!parObj.getparint("verbose",&verbose)) verbose=0;
	if (!parObj.getparint("option",&option)) option=1;
	if (!parObj.getparint("normalize",&normalize)) normalize=0;
