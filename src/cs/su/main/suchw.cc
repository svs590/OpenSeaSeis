...
void* main_suchw( void* args )
{
	...
	/* Get parameters */
	/* get key1's */
	if ((n=parObj.countparval("key1"))!=0){
		nkeys=n;
		parObj.getparstringarray("key1",key1);
	} else { /* set default */
		nkeys=1;
		key1[0]="cdp";	
	}
	...
	/* get key2's */
	if ((n=parObj.countparval("key2"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of key2's and key1's must be equal!");

		parObj.getparstringarray("key2",key2);
	} else { /* set default */
		if (nkeys!=1)
			throw cseis_geolib::csException("number of key2's and key1's must be equal!");
	...
	/* get key3's */
	if ((n=parObj.countparval("key3"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of key3's and key1's must be equal!");

		parObj.getparstringarray("key3",key3);
	} else { /* set default */
		if (nkeys!=1)
			throw cseis_geolib::csException("number of key3's and key1's must be equal!");
		key3[0]="cdp";	
	...
	if ((n=parObj.countparval("a"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of a's and key1's must be equal!");
	...
	if ((n=parObj.countparval("b"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of b's and key1's must be equal!");
	...
	if ((n=parObj.countparval("c"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of c's and key1's must be equal!");
	...
	if ((n=parObj.countparval("d"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of d's and key1's must be equal!");
	...
	/* get e's */
	if ((n=parObj.countparval("e"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of e's and key1's must be equal!");

		e=ealloc1double(nkeys);
		parObj.getpardouble("e",e);
	} else { /* set default */
		e=ealloc1double(nkeys);
		for (ikey=0; ikey<nkeys; ++ikey)
			e[ikey]=1.;
	...
	/* get f's */
	if ((n=parObj.countparval("f"))!=0){
		if (n!=nkeys)
			throw cseis_geolib::csException("number of f's and key1's must be equal!");

		f=ealloc1double(nkeys);
		parObj.getpardouble("f",f);
	} else { /* set default */
		f=ealloc1double(nkeys);
		for (ikey=0; ikey<nkeys; ++ikey)
			f[ikey]=1.;
