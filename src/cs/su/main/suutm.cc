...
void* main_suutm( void* args )
{
    ...
    /* Get parameters */
    if (!parObj.getparint("idx", &idx))         idx=23;
    if (!parObj.getparint("verbose", &verbose)) verbose=0;
    if (!parObj.getparint("counit", &counit))   counit=0;
    if (!parObj.getparstring("zkey", &zkey))    zkey=NULL;
    if (!parObj.getpardouble("xoff", &xoff))    xoff=500000.0;
    if (!parObj.getpardouble("ynoff", &ynoff))  ynoff=0.0;
    if (!parObj.getpardouble("ysoff", &ysoff))  ysoff=10000000.0;
    if (parObj.getpardouble("lon0", &lon0))     userlon0=1;

    /* Check parameters */
    if (idx<0 || idx>=N_ELLIPSOIDS) throw cseis_geolib::csException("unknown ellipsoid idx=%d", idx);
    if (userlon0 && (lon0<-180.0 || lon0>180.0)) \
        throw cseis_geolib::csException("lon0=%g must be between -180 and 180", lon0);
    ...
    if (parObj.getpardouble("a", &a)) {
        if (a<=0.0) throw cseis_geolib::csException("semimajor axis a=%g must be positive", a);
        strcpy(ellip.name, "User-specified\0");
    }
    if (parObj.getpardouble("f", &f)) {
        if (f<0.0) throw cseis_geolib::csException("flattening f=%g must be non-negative", f);
        strcpy(ellip.name, "User-specified\0");
    }    
    parObj.checkpars();
    ...
    while (cs2su->getTrace(&tr)) {
        ...
        if (tr.counit>=2 && tr.counit<=4) {
            ...
            if (gy<-89.999 || gy>89.999 || gx<-180. || gx>=180.) {
                throw cseis_geolib::csException("invalid coordinates: tracl=%d gx=%g gy=%g", \
                    tr.tracl, gx, gy);
            }
            if (sy<-89.999 || sy>89.999 || sx<-180. || sx>=180.) {
                throw cseis_geolib::csException("invalid coordinates: tracl=%d sx=%g sy=%g", \
                    tr.tracl, sx, sy);
            }
...
*************************************************************************/
{
    ...
    break;
    default:
        throw cseis_geolib::csException("unknown type %s", type_out);
    break;
    }
}
