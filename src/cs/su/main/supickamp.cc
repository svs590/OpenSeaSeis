...
void* main_supickamp( void* args )
{
	...
	/* Get parameters */
	if (!parObj.getparfloat("d2", &d2)) d2=0; 
	if (parObj.getparstring("key", &key))	{
	   d2=0;
	} else if(d2==0) {
	   throw cseis_geolib::csException(" Specify d2");
	}

	if (!parObj.getparint("verbose", &verbose))	verbose = 1;
	if (!parObj.getparstring("arg1", &arg1))	arg1 = "max";
	if (!parObj.getparstring("arg2", &arg2))	arg2 = "x2";

	if (!parObj.getparfloat("wl", &wl))	wl = 0.;
	if (!parObj.getparfloat("dt_resamp", &dt_resamp))   dt_resamp = 0.;
	if (!parObj.getparfloat("x2beg", &x2beg))	x2beg = 0.;
	if (!parObj.getparfloat("tmin", &tmin_in))	tmin_in = 0.;

	if (!parObj.getparstring("format", &format)) format="ascii";
	...
	if (parObj.getparstring("t_xabove",&afile)) {
		...
		nx=0;
			...
			nx = parObj.countparval("x_above");
			nt = parObj.countparval("t_above");

		if (nx != nt)
			throw cseis_geolib::csException("lengths of x_above, t_above must be the same");

		x_above = ealloc1float(nx);  parObj.getparfloat("x_above", x_above);
		t_above = ealloc1float(nx);  parObj.getparfloat("t_above", t_above);
	...
	if (parObj.getparstring("t_xbelow",&bfile) ) {

		if (afilep == NULL)
		throw cseis_geolib::csException(" Specify t_xabove -file");
	...
	if(bfilep!= NULL && afilep != NULL) {
	   ...
	   throw cseis_geolib::csException("window not properly specified");
	}


	/* get mode for stdout information */
	if(! get_mode(arg1,&mode1) )
	   throw cseis_geolib::csException(" Unknown arg1 ");

	if(! get_mode(arg2,&mode2) )
	   throw cseis_geolib::csException(" Unknown arg2");

	/* get format for stdout information */
	if(! getformat(format, &modef) )
	   throw cseis_geolib::csException(" Unknown format"); 
	...
	if(! initpick(pick1) )
	   throw cseis_geolib::csException("ERROR initializing local Pick structure");

	/* initialize global picks */
	if(! initpick(gpick) )
	   throw cseis_geolib::csException("ERROR initializing global Pick structure");
	...
	do {
		...
		if(! tracepick(pick1,t_resamp,val_resamp,itime) )
		   throw cseis_geolib::csException(" ERROR in tracepick ");

		/* normalize energy */
		pick1->energy *=dt_resamp;
		
		if(! globalpick(pick1,gpick) )
		   throw cseis_geolib::csException(" ERROR in subroutine globalpick");
		...
		if(! getarg(&t1,mode1,pick1,fval,trnum-1) )
		   throw cseis_geolib::csException(" ERROR in subroutine <getarg>, argument 1");

		if(! getarg(&x1,mode2,pick1,fval,trnum-1) )
		   throw cseis_geolib::csException(" ERROR in subroutine <getarg>, argument 2");
