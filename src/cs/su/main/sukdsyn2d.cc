...
void* main_sukdsyn2d( void* args )
{
	...
	if (!parObj.getparint("nt",&nt)) nt = 501;
	if (!parObj.getparfloat("dt",&dt)) dt = 0.004;
	if (!parObj.getparfloat("ft",&ft)) ft = 0.0;
	if (!parObj.getparfloat("fmax",&fmax)) fmax = 1.0/(4*dt);
	fpeak = 0.2/dt;
	if (!parObj.getparint("nxs",&nxs)) nxs = 101;
	if (!parObj.getparfloat("dxs",&dxs)) dxs = 15;
	...
	if (!parObj.getparfloat("fx",&fx)) throw cseis_geolib::csException("must specify fx!\n");
	if (!parObj.getparfloat("dx",&dx)) throw cseis_geolib::csException("must specify dx!\n");
	if (!parObj.getparint("nz",&nz)) throw cseis_geolib::csException("must specify nz!\n");
	if (!parObj.getparfloat("fz",&fz)) throw cseis_geolib::csException("must specify fz!\n");
	if (!parObj.getparfloat("dz",&dz)) throw cseis_geolib::csException("must specify dz!\n");
	ex = fx+(nx-1)*dx;
	ez = fz+(nz-1)*dz;
	if(fxt>fx || ext<ex || fzt>fz || ezt<ez) 
		throw cseis_geolib::csException(" migration section is out of traveltime table!\n");
