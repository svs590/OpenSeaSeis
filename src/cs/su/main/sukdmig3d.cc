	...
	Memory needed for the ttab:
      ...
      **************************************************************/
      ...
      if (!parObj.getparstring("datain",&datain)) {
	    ...
	    if ((infp=fopen(datain,"r"))==NULL)
		  throw cseis_geolib::csException("cannot open datain=%s",datain);
      ...
      *****************************************************/
      ...
      if ((ttfp=fopen(ttfile,"r"))==NULL)
	    throw cseis_geolib::csException("Can not open ttfile=%s",ttfile);
      if (!fgettr(ttfp,&ttr))
	    throw cseis_geolib::csException("Can't get first trace from tttable");

      if (parObj.getparstring("crfile",&crfile)) {
            if ((gd->crfp=fopen(crfile,"r"))==NULL)
	          throw cseis_geolib::csException("Can not open crfile=%s",crfile);
      }
	else
	gd->crfp=NULL;
	

      if (!parObj.getparfloat("fxgd",&gd->fxgd)) gd->fxgd=ttr.f1;
      if (!parObj.getparfloat("dxgd",&gd->dxgd)) gd->dxgd=ttr.d1;
      if (!parObj.getparint("nxt",&gd->nxt)) 	  gd->nxt=ttr.ns;
      ...
      if (!parObj.getparint("multit",&gd->multit)) gd->multit=ttr.scalel;
      if (gd->multit==0) gd->multit=1;
      gd->nxt/=gd->multit;

      if (!parObj.getparint("ls",&gd->ls)) gd->ls=0;

      if (!parObj.getparfloat("fygd",&gd->fygd)) gd->fygd=ttr.f2;
      if (!parObj.getparfloat("dygd",&gd->dygd)) gd->dygd=ttr.d2;
      if (!parObj.getparint("nyt",&gd->nyt))	  gd->nyt=ttr.ntr;

      gd->nxgd=ttr.selev;
      gd->nygd=ttr.gelev;

      /*number of sources*/
      if (!parObj.getparint("nxs",&gd->nxs))	gd->nxs=ttr.nhs;
      if (!parObj.getparint("nys",&gd->nys))	gd->nys=ttr.nvs;
      if (!parObj.getparint("nzs",&gd->nzs))   gd->nzs=ttr.duse;

      /*first source*/
      if (!parObj.getparint("ixsf",&gd->ixsf))	gd->ixsf=ttr.sdel;
      if (!parObj.getparint("iysf",&gd->iysf))	gd->iysf=ttr.gdel;
      if (!parObj.getparfloat("fzs",&gd->fzs)) gd->fzs=ttr.sdepth/1000.0;

      /*source spacing*/
      if (!parObj.getparint("ixsr",&gd->ixsr))	gd->ixsr=ttr.swdep;
      if (!parObj.getparint("iysr",&gd->iysr))	gd->iysr=ttr.gwdep;
      if (!parObj.getparfloat("dzs",&gd->dzs)) gd->dzs=ttr.ep/1000.0;
      ...
      if (!parObj.getparfloat("dyoffset",&dyoffset)) dyoffset=99999;

      if (!parObj.getparint("ntrmax",&ntrmax)) ntrmax=100000;
      if (!parObj.getparint("pptr",&pptr)) pptr=100;
      ...
      if (!parObj.getparint("nxo",&gd->nxo)) gd->nxo=(gd->nxgd-1)*2+1;
      if (!parObj.getparfloat("fxo",&gd->fxo)) gd->fxo=gd->fxgd;
      if (!parObj.getparfloat("dxo",&gd->dxo)) gd->dxo=gd->dxgd*0.5;

      if (!parObj.getparint("nyo",&gd->nyo)) gd->nyo=(gd->nygd-1)*2+1;
      if (!parObj.getparfloat("fyo",&gd->fyo)) gd->fyo=gd->fygd;
      if (!parObj.getparfloat("dyo",&gd->dyo)) gd->dyo=gd->dygd*0.5;

      if (!parObj.getparint("nzo",&gd->nzo)) gd->nzo=(gd->nzs-1)*5+1;
      if (!parObj.getparfloat("fzo",&gd->fzo)) gd->fzo=gd->fzs;
      if (!parObj.getparfloat("dzo",&gd->dzo)) gd->dzo=gd->dzs*0.2;
      ...
      ***************************************************/
      if (!parObj.getparfloat("xaper",&gd->xaper)) 
       	    gd->xaper=gd->nxt*gd->dxgd/2.5;
      if (!parObj.getparfloat("yaper",&gd->yaper)) 
	    gd->yaper=gd->nyt*gd->dygd/2.5;
      if (!parObj.getparfloat("xoffsetmax",&xoffsetmax)) xoffsetmax=3000;
      if (!parObj.getparfloat("yoffsetmax",&yoffsetmax)) yoffsetmax=3000;

      /**************************************************
      For effective MVA, we need some nxoffset, nyoffset 
      about 10
      **************************************************/
      if (!parObj.getparint("nxoffset",&nxoffset)) nxoffset=1;
      if (!parObj.getparint("nyoffset",&nyoffset)) nyoffset=1;
      if (!parObj.getparfloat("fxoffset",&fxoffset)) fxoffset=0.;
      if (!parObj.getparfloat("fyoffset",&fyoffset)) fyoffset=0.;
      if (!parObj.getparfloat("dxoffset",&dxoffset)) dxoffset=99999;
      ...
      *********************************************************/
      ...
      if (!parObj.getparfloat("dt",&gd->dt)) 
	    gd->dt=((double)tr.dt)/1000.0;  /*ms*/
      if (gd->dt<0.0000001) 
	    throw cseis_geolib::csException("dt must be positive!");
      if (!parObj.getparfloat("ft",&gd->ft)) 
	    gd->ft=tr.delrt/1000.0;
      if (!parObj.getparfloat("dxm",&dxm))
            dxm=tr.d2;
      if (dxm<0.0000001) throw cseis_geolib::csException("dxm must be positive");
      if (!parObj.getparfloat("angmax",&angmax))
            angmax=60.0*PI/180.0;
      if (angmax<0.0000001) throw cseis_geolib::csException("angmax must be positive");
      gd->ntrimax=2*dxm*sin(angmax)/(gd->v0*gd->dt);
      if (gd->ntrimax<1) gd->ntrimax=1;

      if (!parObj.getparfloat("fmax",&gd->fmax)) gd->fmax=0.25/gd->dt;
      ...
      *********************************************************/
	  ...
	  gd->fygd>gd->fyo || eyt<eyo) {
            ...
            fprintf(gd->jpfp,"eyt=%g < eyo=%g ?\n",eyt,eyo);
	    throw cseis_geolib::csException(" migration output range is out of tttable");
      }
	    ...
	    for(ixs=0;ixs<gd->nxs;ixs++){			
		  for (izs=0;izs<gd->nzs;izs++) {
			...
			for (iyt=0;iyt<gd->nyt;iyt++) {
			      if (!fgettr(ttfp,&ttr))
				    throw cseis_geolib::csException("Can't read ttfile");
			      for (imul=0;imul<gd->multit;imul++) {
					  ...
					  sizeof(float),gd->nxt,gd->crfp)!=gd->nxt) 
				          throw cseis_geolib::csException("Can not read in crfile");
				    for (ixt=0;ixt<gd->nxt;ixt++) 
