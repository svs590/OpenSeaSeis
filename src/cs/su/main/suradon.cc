...
void* main_suradon( void* args )
{
	...
	if (!parObj.getparint("xopt",&xopt))	xopt=1;
	if (!parObj.getparstring("cdpkey",&cdpkey)) cdpkey="cdp";
	if (!parObj.getparstring("offkey",&offkey)) offkey="offset";
	if (!parObj.getparint("choose",&choose)) choose=0;
	if (!parObj.getparint("ninterp",&ninterp)) ninterp=0;
	if (!parObj.getparint("nwin",&nk)) nk=1;
	if (!parObj.getparint("igopt",&igopt)) igopt=1;
	if (!parObj.getparint("nxmax",&nxmax)) nxmax=240;
	if (!parObj.getparint("ninterp",&ninterp)) ninterp=1;
	if (!parObj.getparint("lagc",&lagc))lagc=400;
	if (!parObj.getparint("lent",&lent)) lent=5;
	if (!parObj.getparint("lenx",&lenx)) lenx=7;
	if (!parObj.getparfloat("freq1",&freq1)) freq1=4.;
	if (!parObj.getparfloat("freq2",&freq2)) freq2=20.;
	if (!parObj.getparfloat("offref",&offref)) offref=2000.;
	if (!parObj.getparfloat("f1",&f1)) f1=60.;
	if (!parObj.getparfloat("f2",&f2)) f2=80.;
	if (!parObj.getparfloat("pmin",&pmin)) pmin=-200.;
	if (!parObj.getparfloat("pmax",&pmax)) pmax=400.;
	if (!parObj.getparfloat("dp",&dp)) dp=16.;
	if (!parObj.getparfloat("pmula",&pmula)) pmula=80.;
	if (!parObj.getparfloat("pmulb",&pmulb)) pmulb=200.;
	if (!parObj.getparfloat("depthref",&depthref)) depthref=500.;
	if (!parObj.getparfloat("interoff",&intercept_off)) intercept_off=0.;
	if (!parObj.getparfloat("prewhite",&prewhite)) prewhite=0.1;
	if (!parObj.getparint("ltaper",&ltaper)) ltaper=7;
	...
	ipb=( pmulb -  pmin)/ dp;
	np=1+( pmax -  pmin)/ dp;
	if(np<1)throw cseis_geolib::csException("Range of PMIN and PMAX invalid");
	if(choose==0) ipa=0;
	if(choose==3) ipa=0;
	if(choose==4) ipa=0;
	if(ipa<0) ipa=0;
	...
	if( (headerfp = fopen(headerfile,"w+"))==NULL)
		throw cseis_geolib::csException("couldn't open temp file for trace headers");
