...
void* main_sumiggbzoan( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sumiggbzoan );
	...
	CSMUSTGETPARSTRING("a3333file",&a3333file);
	CSMUSTGETPARINT("nx",&nx);
	CSMUSTGETPARINT("nz",&nz);
	...
	if (!parObj.getparfloat("dz",&dz)) dz = 1.0;
	if (!parObj.getparfloat("fmin",&fmin)) fmin = 0.025/dt;
	if (!parObj.getparfloat("fmax",&fmax)) fmax = 10.0*fmin;
	if (!parObj.getparfloat("amax",&amax)) amax = 60.0;
	if (!parObj.getparfloat("amin",&amin)) amin = -amax;
	...
	if (!parObj.getparstring("a1111file",&a1111file)) 
			ia1111=0;
	if (!parObj.getparstring("a1133file",&a1133file)) 
			ia1133=0;
	if (!parObj.getparstring("a1313file",&a1313file)) 
			ia1313=0;
	if (!parObj.getparstring("a1113file",&a1113file)) 
			ia1113=0;
	if (!parObj.getparstring("a3313file",&a3313file)) 
			ia3313=0;
	if (!parObj.getparstring("deltafile",&deltafile)) 
			idelta=0;
	if (!parObj.getparstring("epsilonfile",&epsilonfile)) 
			iepsilon=0;

	
	/* read and halve required velocities*/
	if ((a3333fp=fopen(a3333file,"r"))==NULL)
		throw cseis_geolib::csException("error opening a3333file=%s!\n",a3333file);
	if (fread(a3333[0],sizeof(float),nz*nx,a3333fp)!=nz*nx)
		throw cseis_geolib::csException("error reading a3333file=%s!\n",a3333file);

	/* read and halve velocities*/
	if(ia1111){
		if ((a1111fp=fopen(a1111file,"r"))==NULL)
			throw cseis_geolib::csException("error opening a1111file=%s!\n",a1111file);
		if (fread(a1111[0],sizeof(float),nz*nx,a1111fp)!=nz*nx)
			throw cseis_geolib::csException("error reading a1111file=%s!\n",a1111file);
	}else{
		for (ix=0; ix<nx; ++ix)
			for (iz=0; iz<nz; ++iz)
	...
	if(ia1133){
		if ((a1133fp=fopen(a1133file,"r"))==NULL)
			throw cseis_geolib::csException("error opening a1133file=%s!\n",a1133file);
		if (fread(a1133[0],sizeof(float),nz*nx,a1133fp)!=nz*nx)
			throw cseis_geolib::csException("error reading a1133file=%s!\n",a1133file);
	}else{
		for (ix=0; ix<nx; ++ix)
			for (iz=0; iz<nz; ++iz)
	...
	if(ia1313){
		if ((a1313fp=fopen(a1313file,"r"))==NULL)
			throw cseis_geolib::csException("error opening a1313file=%s!\n",a1313file);
		if (fread(a1313[0],sizeof(float),nz*nx,a1313fp)!=nz*nx)
			throw cseis_geolib::csException("error reading a1313file=%s!\n",a1313file);
	}else{
		for (ix=0; ix<nx; ++ix)
			for (iz=0; iz<nz; ++iz)
	...
	if(ia1113){
		if ((a1113fp=fopen(a1113file,"r"))==NULL)
			throw cseis_geolib::csException("error opening a1113file=%s!\n",a1113file);
		if (fread(a1113[0],sizeof(float),nz*nx,a1113fp)!=nz*nx)
			throw cseis_geolib::csException("error reading a1113file=%s!\n",a1113file);
	}else{
		for (ix=0; ix<nx; ++ix)
			for (iz=0; iz<nz; ++iz)
				a1113[ix][iz] = 0;
	...
	if(ia3313){
		if ((a3313fp=fopen(a3313file,"r"))==NULL)
			throw cseis_geolib::csException("error opening a3313file=%s!\n",a3313file);
		if (fread(a3313[0],sizeof(float),nz*nx,a3313fp)!=nz*nx)
			throw cseis_geolib::csException("error reading a3313file=%s!\n",a3313file);
	}else{
		for (ix=0; ix<nx; ++ix)
			for (iz=0; iz<nz; ++iz)
				a3313[ix][iz] = 0;
	...
	if(idelta==1 || iepsilon==1) {
		if(idelta){
			if ((deltafp=fopen(deltafile,"r"))==NULL)
				throw cseis_geolib::csException("error opening deltafile=%s!\n",deltafile);
			if (fread(delta[0],sizeof(float),nz*nx,deltafp)!=nz*nx)
				throw cseis_geolib::csException("error reading deltafile=%s!\n",deltafile);
		}else{
			for (ix=0; ix<nx; ++ix)
				for (iz=0; iz<nz; ++iz)
					delta[ix][iz] = 0;
		...
		if(iepsilon){
			if ((epsilonfp=fopen(epsilonfile,"r"))==NULL)
				throw cseis_geolib::csException("error opening epsilonfile=%s!\n",epsilonfile);
			if (fread(epsilon[0],sizeof(float),nz*nx,epsilonfp)!=nz*nx)
				throw cseis_geolib::csException("error reading epsilonfile=%s!\n",epsilonfile);
		}else{
			for (ix=0; ix<nx; ++ix)
				for (iz=0; iz<nz; ++iz)
					epsilon[ix][iz] = 0;
	...
	/* get beam half-width */
	if (!parObj.getparfloat("bwh",&bwh)) bwh = vavg/fmin;
