...
void* main_las2su( void* args )
{
	...
	if (!parObj.getparint("m", &m)) 	m = 0;
	if (!parObj.getparint("ss", &ss)) 	ss = 0;
	if (!parObj.getparfloat("dz", &dz)) 	dz = 0.5;
	if (!parObj.getparstring("outhdr", &outhdr))  outhdr = LAS_HDR_DEF;
	outhdrfp = efopen(outhdr, "w");

	/* WMH: read LAS header, extract ncurve, and copy to outhdr */
	/* hardyr: flip the booleans here */
	if (!parObj.getparint("ncurve",&ncurve)) {
