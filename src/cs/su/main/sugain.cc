...
void* main_sugain( void* args )
{
	...
	/* Get parameters */
	if (!parObj.getparfloat ("tpow" , &tpow))	tpow     = TPOW;
	if (!parObj.getparfloat ("epow" , &epow))	epow     = EPOW;
	if (!parObj.getparfloat ("gpow" , &gpow))	gpow     = GPOW;
	if (!parObj.getparfloat ("vred" , &vred))	vred     = VRED;
	if (!parObj.getparfloat ("trap" , &trap))	trap     = TRAP;
	if (!parObj.getparfloat ("clip" , &clip))	clip     = CLIP;
	if (!parObj.getparfloat ("pclip" , &pclip))    pclip    = FLT_MAX;
	if (!parObj.getparfloat ("nclip" , &nclip))    nclip    = -FLT_MAX;
	if (!parObj.getparfloat ("qclip", &qclip))     qclip    = QCLIP;
	if (!parObj.getparfloat ("scale", &scale))     scale    = SCALE;
	if (!parObj.getparfloat ("norm",  &norm)) 	norm     = 0.0;
	if (!parObj.getparfloat ("bias",  &bias))	bias     = BIAS;
	if (!parObj.getparfloat ("wagc" , &wagc))	wagc     = WAGC;
	if (!parObj.getparint   ("agc"  , &agc))	agc	= 0;
	if (!parObj.getparint   ("gagc" , &gagc))	gagc     = 0;
	if (!parObj.getparint   ("pbal" , &pbal))	pbal     = 0;
	if (!parObj.getparint   ("qbal" , &qbal))	qbal     = 0;
	if (!parObj.getparint   ("mbal" , &mbal))	mbal     = 0;
	if (!parObj.getparint   ("panel", &panel))     panel    = 0;
	if (!parObj.getparint   ("jon"  , &jon))	jon	= 0;
	if (!parObj.getparint("verbose", &verbose))	verbose  = 0;
	if (!parObj.getparint   ("maxbal" , &maxbal))	maxbal   = 0;
	if (!parObj.getparint   ("mark" , &mark))	mark     = 0;
	
        parObj.checkpars();

	/* Data validation */
	if (vred < 0.0) throw cseis_geolib::csException("vred = %f, must be positive", vred);
	if (trap < 0.0) throw cseis_geolib::csException("trap = %f, must be positive", trap);
	if (clip < 0.0) throw cseis_geolib::csException("clip = %f, must be positive", clip);
	if (qclip < 0.0 || qclip > 1.0) 
		throw cseis_geolib::csException("qclip = %f, must be between 0 and 1", qclip);
	if (agc || gagc) {
		iwagc = NINT(wagc/dt);
		if (iwagc < 1) throw cseis_geolib::csException("wagc=%g must be positive", wagc);
		if (iwagc > nt) throw cseis_geolib::csException("wagc=%g too long for trace", wagc);
