...
void* main_suinvvxzco( void* args )
{
	...
	su2cs->setSUDoc( sdoc_suinvvxzco );
	...
	if (!parObj.getparfloat("offs",&offs)) offs = tr.offset;
	if (!parObj.getparfloat("dxm",&dxm)) dxm = tr.d2;
	if  (dxm<0.0000001) throw cseis_geolib::csException("dxm must be positive!\n");
	
	/* get required parameters */
	if (!parObj.getparint("nx",&nx)) throw cseis_geolib::csException("must specify nx!\n");
	if (!parObj.getparint("nz",&nz)) throw cseis_geolib::csException("must specify nz!\n");
	if (!parObj.getparint("nxm",&nxm)) throw cseis_geolib::csException("must specify nxm!\n");
	if (!parObj.getparstring("vfile",&vfile)) throw cseis_geolib::csException("must specify vfile!\n");
	
	/* get optional parameters */
	if (!parObj.getparfloat("fxm",&fxm)) fxm = 0.0;
	if (!parObj.getparint("nxd",&nxd)) nxd = 5;
	if (!parObj.getparfloat("dx",&dx)) dx = 50.;
	if (!parObj.getparfloat("fx",&fx)) fx = 0.0;
	if (!parObj.getparfloat("dz",&dz)) dz = 50.;
	if (!parObj.getparint("nxb",&nxb)) nxb = nx/2;
	if (!parObj.getparint("nxc",&nxc)) nxc = 0;
	nxc = MIN(nxc,nxb);
	if (!parObj.getparint("nzc",&nzc)) nzc = 0;
	nzc = MIN(nzc,nz);
	if (!parObj.getparfloat("fxo",&fxo)) fxo = 0.0;
	/* check the first output trace	*/
	if (fxo<fx )
		throw cseis_geolib::csException("the first output trace is out of velocity model.\n");
	if (!parObj.getparfloat("dxo",&dxo)) dxo = 15.;
	if (!parObj.getparint("nxo",&nxo)) nxo = 101;
	if (!parObj.getparfloat("fzo",&fzo)) fzo = 0.;
	if (!parObj.getparfloat("dzo",&dzo)) dzo = 15.;
	if (!parObj.getparint("nzo",&nzo)) nzo = 101;
	if (!parObj.getparint("ls",&ls)) ls = 0;
	if (!parObj.getparint("pert",&pert)) pert = 0;
	if (!parObj.getparint("verbose",&verbose)) verbose = 1;
	if (!parObj.getparfloat("fmax",&fmax)) fmax = 0.25/dt;
	if (!parObj.getparfloat("ang",&ang)) ang = 180;
	cosa = cos(ang*PI/180);
	...
	ez = (nz-1)*dz;
 	...
 	for (ixm=0; ixm<nxm; ++ixm) {
		...
		throw cseis_geolib::csException("shot or receiver lie outside of specified (x,z) grid\n");
	} 
	...
	if(fread(vold[0],sizeof(float),nx*nz,fopen(vfile,"r"))!=nx*nz)
		throw cseis_geolib::csException("cannot read %d velocities from file %s",nx*nz,vfile);
	if(pert) {
		/* read slowness perturbation*/
		if (!parObj.getparstring("vpfile",&vpfile)) 
			throw cseis_geolib::csException("must specify vpfile!\n");
		if(fread(vpold[0],sizeof(float),nx*nz,fopen(vpfile,"r"))
			!=nx*nz)
		throw cseis_geolib::csException("cannot read %d slowness perturbation from file %s"
			,nx*nz,vpfile);
	...
	if(ABS(offs)>2*nxb*dx) throw cseis_geolib::csException("\t band NXB is too small!");
	...
	for (ixm=0; ixm<nxm; ixm+=nxd1){
		...
		for (ixo=0,xo=fxo; ixo<nxo; ++ixo,xo+=dxo) 
			...
			if(ixm<nxm-1) cs2su->getTrace(&tr);
	    }
	    else {
		...
		for (ixo=0,xo=fxo; ixo<nxo; ++ixo,xo+=dxo) 
			...
			/* read input trace */
			if(ixm-nxd1+i<nxm-1) cs2su->getTrace(&tr);
