...
void* main_sualford( void* args )
{
	...
	if(!parObj.getparint("ntraces", &ntraces)) ntraces=5;


	/* getting the names of the input and output files */
	if (!parObj.getparstring("inS11",&inS11))
		throw cseis_geolib::csException("no file specified for S11 volume. Aborting...");	
	if (!parObj.getparstring("inS12",&inS12))
		throw cseis_geolib::csException("no file specified for S12 volume. Aborting...");
	if (!parObj.getparstring("inS21",&inS21))
		throw cseis_geolib::csException("no file specified for S21 volume. Aborting...");
	if(!parObj.getparstring("inS22",&inS22))
		throw cseis_geolib::csException("no file specified for S22 volume. Aborting...");
	if(!parObj.getparstring("outS11",&outS11))
		outS11="S11_out";	
	if(!parObj.getparstring("outS12",&outS12))
		outS12="S12_out";	
	if (!parObj.getparstring("outS21",&outS21))
		outS21="S21_out";
	if (!parObj.getparstring("outS22",&outS22))
		outS22="S22_out";
	...
	if (!parObj.getparstring("Az_key",&Az_key)) 		Az_key = "otrav";
	if (!parObj.getparstring("Q_key",&Q_key)) 		Q_key = "grnlof";
	if (!parObj.getparstring("lag_key",&lag_key))          lag_key = "tstat";
	if (!parObj.getparstring("xcorr_key", &xcorr_key))     xcorr_key = "grnors";
	if (!parObj.getparstring("INPUTQ_key", &INPUTQ_key))   INPUTQ_key= "gaps";
	...
	******************************************************************/
	...
	if (parObj.getparfloat("tmin", &tmin))
	...
	if (parObj.getparfloat("tmax", &tmax)){
		...
		if (itmin <0)
			throw cseis_geolib::csException("itmin = %d should be positive", itmin);
		if (nt > SU_NFLTS)
			throw cseis_geolib::csException(" nt = %d exceeds SU_NFLTS=%d", nt, SU_NFLTS);
		if (itmin > itmax)
			throw cseis_geolib::csException("itmin=%d, itmax = %d conflict", itmin, itmax);
	...
	********************************************************************/
	...
	if(nS11 != nS22 || nS11 != nS12 || nS11 != nS21){
	  throw cseis_geolib::csException("trace length for S11 is not equal to other traces \n");
	}
	if(nS22 != nS12 || nS22 != nS21){
	    throw cseis_geolib::csException("trace length for S22 is not equal to other traces\n");
	}
	if(nS12 != nS21){
	  throw cseis_geolib::csException("trace length for off diagonals is not the same\n");
	}
...
***************************************************************************/
{
     ...
     cos2   = pow(cos(rad),2.0);
     sin2   = pow(sin(rad),2.0);
...
/* Convert output to appropriate type */
	...
	break;
	default:
		throw cseis_geolib::csException("unknown type %s", type_in);
	break;
	}
}
...
*****************************************************************/
{
	...
	/* verifying the existence of input file */  
	    if ((fp = fopen(vol_name,"r")) == NULL)
	      {
		throw cseis_geolib::csException("cannot open volume %s\n",vol_name);
	      }
	/* getting first trace from input file */
	    if (!fgettr(fp,&tr2))
	      {
		throw cseis_geolib::csException("cannot read first trace in %s volume\n",vol_name);
	      }
	/* getting number of samples and sampling rate in sec */
	    nt = (int)tr2.ns;
	   *dt = ((double)tr2.dt)/1000000.0;
	   *ns=nt; 
	    if (dt == 0) throw cseis_geolib::csException("dt not set in header for %s volume", vol_name);
	   *tr=tr2;
	return fp;   
...
****************************************************************/
{
    ...
    if ((fp = fopen(filename,"w")) == NULL)
      {
	throw cseis_geolib::csException("cannot open output volume %s file \n",filename);
      }
	return fp;
}
