...
void* main_sutihaledmo( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sutihaledmo );
	...
	if (!parObj.getparfloat("v",&v)) v = 1500.0;
	if (!parObj.getparfloat("h",&h)) h = 200.0;
	if (!parObj.getparfloat("e",&e)) e = 0.0;
	if (!parObj.getparfloat("d",&d)) d = 0.0;
	if (!parObj.getparint("nxpad",&nxpad)) nxpad = h/dx;
	if (!parObj.getparint("ntpad",&ntpad)) ntpad = 0;
	if (!parObj.getparint("option",&option)) option = 1;
	if (!parObj.getparstring("file",&file)) file="vnmo";
	if (v <= 0.0 && (option==3 || option==6)) 
		throw cseis_geolib::csException("must enter positive value for v when option=3"); 
	
	/* get required parameters */
	if (!parObj.getparint("nxmax",&nxmax)) throw cseis_geolib::csException("must enter nxmax");
	...
	if(option==5) {
		if((fp=efopen(file,"r"))==NULL)
		   throw cseis_geolib::csException("suhaledmo: couldn't open vnmo(p) file, option=5");
		fscanf(fp,"%d",&np);
