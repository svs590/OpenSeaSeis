...
void* main_susort( void* args )
{
	...
	if (ispipe && ftypeout != DISK)
		throw cseis_geolib::csException("OK IO types are: DISK->ANYTHING, PIPE->DISK");
	...
	ntr = 0;
	if (!(nsegy = cs2su->getTrace(&tr)))  throw cseis_geolib::csException("can't get first trace");
...
Value negval(cwp_String type, Value val)
{
	...
	case 'd':
		val.d = -val.d;
	break;
	default: throw cseis_geolib::csException("%d: mysterious type %s", __LINE__, type);
	}
