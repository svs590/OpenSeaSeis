...
void* main_suhrot( void* args )
{
	...
	user=1;
	if ((nav=parObj.countparval("a"))) {
		if (!(nxv = parObj.countparval("x"))) {
			nxv=1;
			x=ealloc1float(1); x[0]=0.0;
		} else {
			x=ealloc1float(nxv); parObj.getparfloat("x", x);
		}
		if (nav != nxv) 
			throw cseis_geolib::csException("number of values in \"x=\" and \"a=\" must be equal");
			
		a=ealloc1float(nav); parObj.getparfloat("a", a);
	} else if (parObj.getparstring("afile", &afile)) {
		CSMUSTGETPARSTRING("xfile",&xfile);
		CSMUSTGETPARINT("n",&nxv);
		nav = nxv;
		a = ealloc1float(nav);
		x = ealloc1float(nxv);

		if(!(afilep=fopen(afile,"r"))) 
			throw cseis_geolib::csException("cannot open afile=%s",afile);
		if (fread(a,sizeof(float),nav,afilep) != nav) 
			throw cseis_geolib::csException("error reading afile=%s",afile);

		if(!(xfilep=fopen(xfile,"r"))) 
			throw cseis_geolib::csException("cannot open xfile=%s",xfile);
		if (fread(x,sizeof(float),nxv,xfilep) != nxv) 
			throw cseis_geolib::csException("error reading xfile=%s",xfile);
