...
void* main_sudmotx( void* args )
{
	...
	if (cdpmin>cdpmax) throw cseis_geolib::csException("cdpmin must be less than cdpmax");
	if (!parObj.getparfloat("dxcdp",&dxcdp)) throw cseis_geolib::csException("must specify dxcdp");
	if (!parObj.getparint("noffmix",&noffmix)) throw cseis_geolib::csException("must specify noffmix");
	if (!parObj.getparfloat("offmax",&offmax)) offmax=3000.0;
	if (!parObj.getparfloat("tmute",&tmute)) tmute=2.0;
	if (!parObj.getparfloat("vrms",&vrms)) vrms=1500.0;
	if (!parObj.getparint("nsmax",&nsmax)) nsmax=400;
