...
void* main_susynvxz( void* args )
{
	...
	if (!parObj.getparint("nxb",&nxb)) nxb = nx;
	if (!parObj.getparint("nxd",&nxd)) nxd = 1;
	if (!parObj.getparfloat("dx",&dx)) dx = 100;
	if (!parObj.getparfloat("fx",&fx)) fx = 0.0;
	if (!parObj.getparfloat("dz",&dz)) dz = 100;
	...
	if (!parObj.getparfloat("dxo",&dxo)) dxo = 50;
	if (!parObj.getparfloat("fxo",&fxo)) fxo = 0.0;
	if (!parObj.getparint("nxm",&nxm)) nxm = 101;
	if (!parObj.getparfloat("dxm",&dxm)) dxm = 50;
	...
	for (ixo=0, tracl=0; ixo<nxo; ++ixo){
	    xo = fxo+ixo*dxo;
	    if(ABS(xo)>nxb*dx) throw cseis_geolib::csException("\t band NXB is too small!\n");
	    nxd1 = nxd;
