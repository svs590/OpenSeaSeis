...
void* main_sumigpresp( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sumigpresp );
	...
	if (!parObj.getparfloat("fmax",&fmax)) fmax = 25. ;  
	...
	if (!parObj.getparfloat("dx",&dx)) {
		...
		*/
		...
		cq = alloc2complex(nw,nxo);
                ...
                oldgxmax=gxmax;
                oldgxmin=gxmin;
                while(cs2su->getTrace(&tr)) { /* begin looping over traces within a shot gather */
