...
void* main_sufctanismod( void* args )
{
  ...
  su2cs->setSUDoc( sdoc_sufctanismod );
  ...
  if (!parObj.getparint("nt", &nt)) nt=250;
  if (!parObj.getparint("nx", &nx)) nx=300;
  if (!parObj.getparint("nz", &nz)) nz=250;
  if (!parObj.getparint("sx", &sx)) sx=nx/2;
	if (sx>nx) throw cseis_geolib::csException("Source sx=%i is greater than nx=%i. Exit",sx,nx);
  if (!parObj.getparint("sz", &sz)) sz=nz/2;
	if (sz>nz) throw cseis_geolib::csException("Source sz=%f is greater than nz=%i. Exit",sz,nx);
	if (!parObj.getparint("receiverdepth", &receiverdepth)) receiverdepth=sz;
  if (!parObj.getparint("vspnx", &vspnx)) vspnx=sx;
  if (!parObj.getparint("impulse", &impulse)) impulse=0;
  if (!parObj.getparint("source", &source)) source=0;
  if (!parObj.getparint("isurf", &isurf)) isurf=1;
  if (!parObj.getparint("dofct", &dofct)) dofct=0;
  if (!parObj.getparint("fctxbeg", &fctxbeg)) fctxbeg=0;
  if (!parObj.getparint("fctzbeg", &fctzbeg)) fctzbeg=0;
  if (!parObj.getparint("fctxend", &fctxend)) fctxend=nx;
  if (!parObj.getparint("fctzend", &fctzend)) fctzend=nz;
  if (!parObj.getparint("indexux", &indexux)) indexux=0;
  if (!parObj.getparint("indexuy", &indexuy)) indexuy=0;
  if (!parObj.getparint("indexuz", &indexuz)) indexuz=0;
  if (!parObj.getparint("wavelet", &wavelet)) wavelet=1;
  if (!parObj.getparint("movebc", &movebc)) movebc=0;
  if (!parObj.getparint("order", &order)) order=2;
  if (!parObj.getparint("suhead",&suhead)) suhead=1;
  if (!parObj.getparint("mt",&mt)) mt=1;
  if (!parObj.getparint("verbose",&verbose)) verbose=0;
  if (!parObj.getparfloat("dx", &dx)) dx=0.02;
  if (!parObj.getparfloat("dz", &dz)) dz=0.02;
  if (!parObj.getparfloat("dt", &dt)) dt=0.002;
  if (!parObj.getparfloat("fpeak", &fpeak)) fpeak=20.0;
  if (!parObj.getparfloat("aa00", &aa00)) aa00=2.0;
  if (!parObj.getparfloat("cc00", &cc00)) cc00=2.0;
  if (!parObj.getparfloat("ff00", &ff00)) ff00=2.0;
  if (!parObj.getparfloat("ll00", &ll00)) ll00=2.0;
  if (!parObj.getparfloat("nn00", &nn00)) nn00=2.0;
  if (!parObj.getparfloat("daadx", &daadx)) daadx=0.0;
  if (!parObj.getparfloat("daadz", &daadz)) daadz=0.0;
  if (!parObj.getparfloat("dccdx", &dccdx)) dccdx=0.0;
  if (!parObj.getparfloat("dccdz", &dccdz)) dccdz=0.0;
  if (!parObj.getparfloat("dffdx", &dffdx)) dffdx=0.0;
  if (!parObj.getparfloat("dffdz", &dffdz)) dffdz=0.0;
  if (!parObj.getparfloat("dlldx", &dlldx)) dlldx=0.0;
  if (!parObj.getparfloat("dlldz", &dlldz)) dlldz=0.0;
  if (!parObj.getparfloat("dnndx", &dnndx)) dnndx=0.0;
  if (!parObj.getparfloat("dnndz", &dnndz)) dnndz=0.0;
  if (!parObj.getparfloat("drhodx", &drhodx)) drhodx=0.0;
  if (!parObj.getparfloat("drhodz", &drhodz)) drhodz=0.0;
  if (!parObj.getparfloat("rho00", &rho00)) rho00=1.0;
  if (!parObj.getparfloat("eta0", &eta0)) eta0=0.03;
  if (!parObj.getparfloat("eta", &eta)) eta=0.04;
  if (!parObj.getparfloat("deta0dx", &deta0dx)) deta0dx=0.0;
  if (!parObj.getparfloat("deta0dz", &deta0dz)) deta0dz=0.0;
  if (!parObj.getparfloat("detadx", &detadx)) detadx=0.0;
  if (!parObj.getparfloat("detadz", &detadz)) detadz=0.0;
  parObj.getparstring("sfile",&sfile);
  parObj.getparstring("dfile",&dfile);
  parObj.getparstring("afile",&afile);
  parObj.getparstring("cfile",&cfile);
  parObj.getparstring("ffile",&ffile);
  parObj.getparstring("lfile",&lfile);
  parObj.getparstring("nfile",&nfile);
  parObj.getparstring("nfile",&nfile);
  parObj.getparstring("reflxfile",&reflxfile);
  parObj.getparstring("reflyfile",&reflyfile);
  parObj.getparstring("reflzfile",&reflzfile);
  parObj.getparstring("vspxfile",&vspxfile);
  parObj.getparstring("vspyfile",&vspyfile);
  parObj.getparstring("vspzfile",&vspzfile);
