...
void* main_sumigprepspi( void* args )
 {
   ...
   su2cs->setSUDoc( sdoc_sumigprepspi );
   ...
   CSMUSTGETPARFLOAT("dz",&dz);
   CSMUSTGETPARSTRING("vfile", &vfile);
   CSMUSTGETPARINT("nxo",&nxo);
   CSMUSTGETPARINT("nxshot",&nxshot);
   
   /* get optional parameters */
   if (!parObj.getparfloat("fmax",&fmax)) fmax = 25.0;  
   if (!parObj.getparfloat("f1",&f1)) f1 = 10.0;
   if (!parObj.getparfloat("f2",&f2)) f2 = 20.0;
   if (!parObj.getparfloat("f3",&f3)) f3 = 40.0;
   if (!parObj.getparfloat("f4",&f4)) f4 = 50.0;
   
   if (!parObj.getparint("lpad",&lpad)) lpad=9999;
   if (!parObj.getparint("rpad",&rpad)) rpad=9999;
   if (!parObj.getparint("dip",&dip)) dip=65;

   if (!parObj.getparint("nflag",&nflag)) nflag=0;
   ...
   do {    /* begin loop over shots */
     ...
     */
	 ...
	 warn(" inside loop:  min_sx_gx %f isx %d igx %d gx %f sx %f",min_sx_gx,isx,igx,gx,sx);
       ...
       if (!(oldsx <= sx) )
	 throw cseis_geolib::csException("sx field must be monotonically increasing!");
       if (!(oldgx <= gx) )
	 throw cseis_geolib::csException("gx field must be monotonically increasing!");
       
       ++nx;
     } while(cs2su->getTrace(&tr) && sx==oldsx);
