...
void* main_suwaveform( void* args )
{
	...
	su2cs->setSUDoc( sdoc_suwaveform );
	...
	if (dt<=0.0) throw cseis_geolib::csException("dt=%g must be positive", dt);
	if (!parObj.getparfloat("fpeak", &fpeak)) fpeak=20.0;
	if (fpeak<=0.0) throw cseis_geolib::csException("fpeak=%g must be positive", fpeak);
	if (!parObj.getparfloat("half", &half)) half=1.0/fpeak;
	if (half<=0.0) throw cseis_geolib::csException("half=%g must be positive", half);
	if (!parObj.getparfloat("period", &period)) period=half*0.77969680123;
	if (period<=0.0) throw cseis_geolib::csException("period=%g must be positive", period);
	if (!parObj.getparfloat("tspike", &tspike)) tspike=0.0;
	if (tspike<0.0) throw cseis_geolib::csException("tspike=%g must be non-negative", tspike);	
	if (!parObj.getparfloat("ampl", &ampl)) ampl=1.0;
	if (!parObj.getparfloat("distort", &distort)) distort=0.0;
	if (!parObj.getparfloat("tn", &tn)) tn=2.0;
	if (tn<0.0) throw cseis_geolib::csException("tn=%g must be non-negative", tn);  
	if (!parObj.getparfloat("decay", &decay)) decay=4.0*fpeak;
	if (decay<0.0) throw cseis_geolib::csException("decay=%g must be non-negative", decay);  
	if (!parObj.getparfloat("ipa", &ipa)) ipa=-90.0;
	ipa *= PI/180.0;

	if (!parObj.getparint("verbose", &verbose)) verbose=0;
	if (!parObj.getparstring("type", &wtype)) wtype="akb";

	/* Alias "ricker" for "ricker1" */
	if (STREQ(wtype,"ricker")) wtype="ricker1";
	
	/* Get number of samples or estimate "useful" trace length */
	if (!parObj.getparint("ns", &nt) && !getparint("nt", &nt)) {
	if (STREQ(wtype,"berlage")) {
		...
		throw cseis_geolib::csException("unknown wavelet type=%s", wtype);
	}
	};
