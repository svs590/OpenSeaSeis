...
void* main_sustaticrrs( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sustaticrrs );
	...
	if (hdrs == 2){
        ...
        if (hdrs==3){
                parObj.getparstring("blvl_file",&blvl_file);
                parObj.getparstring("refr_file",&refr_file);
                parObj.getparint("nsamp",&nsamp);
                parObj.getparfloat("fx",&fx);
                parObj.getparfloat("dx",&dx);
                parObj.getparfloat("V_r",&V_r);

                blvl_arr = alloc1float(nsamp);
                if((fpb=efopen(blvl_file,"rb"))==NULL)
                        throw cseis_geolib::csException("cannot open blvl_file=%s\n",blvl_file);
                efread(blvl_arr, sizeof(float),nsamp,fpb);

                refr_arr = alloc1float(nsamp);
                if((fpf=efopen(refr_file,"rb"))==NULL)
                        throw cseis_geolib::csException("cannot open refr_file=%s\n",refr_file); 
                ...
                if (!parObj.getparfloat("dzv",&dzv)) throw cseis_geolib::csException("must specify dzv");
                vfp = fopen(vfile,"r");
        ...
        if (hdrs==4){
                ...
                if((g3=efopen("tcdp.b","rb"))==NULL)
                        throw cseis_geolib::csException("cannot open tcdp.b\n");
                efread(tcdp_arr, sizeof(float),nsamp,g3);

                num_arr = alloc1float(nsamp);
                if((g4=efopen("num.b","rb"))==NULL)
                        throw cseis_geolib::csException("cannot open num.b\n");
