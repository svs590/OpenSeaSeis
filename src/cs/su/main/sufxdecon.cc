...
void* main_sufxdecon( void* args )
{
	...
	if (!parObj.getparfloat("taper", &taper)) taper=.1;
	if (taper==0.0) taper=.004; 

	if (!parObj.getparfloat("fmin", &fmin)) fmin= 6.;
	if (fmin==0.0) 
	        if (verbose) warn("using fmin=6 Hz");

	if (!parObj.getparfloat("fmax", &fmax)) fmax=.6/(2*dt);
	if (fmax==0.0) 
	        if (verbose) warn("using fmax=.6/(2*dt) Hz");

	if (!parObj.getparfloat("twlen", &twlen)) twlen=(float)(nsp-1)*dt;
	if (twlen<.3) {
	        twlen=.3;
	...
	if (!parObj.getparint("ntrw", &ntrw)) ntrw=10;
	if (!ntrw) {
	        ntrw = 10;
	...
	if (!parObj.getparint("ntrf", &ntrf)) ntrf=4;
	if (!ntrf) 
...
/*
	if (nfft >= SU_NFLTS || nfft >= PFA_MAX)  
	        throw cseis_geolib::csException("Padded nsp=%d--too big", nfft);
*/
	if (nfft >= PFA_MAX)  
	        throw cseis_geolib::csException("Padded nsp=%d--too big", nfft);
