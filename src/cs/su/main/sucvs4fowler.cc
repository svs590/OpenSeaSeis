...
void* main_sucvs4fowler( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sucvs4fowler );
	...
	if (!parObj.getparfloat("vminstack",&vmin)) vmin=1500.;
	if (!parObj.getparfloat("vhyp",&vhyp)) vhyp=2500.;
	if (!parObj.getparint("nvstack", &nvstack)) nvstack = 180;
	if (!parObj.getparint("lmute", &lmute)) lmute = 24;
	lmute=1 + 0.001*lmute/dtout;	
	if (!parObj.getparint("ncdps", &ncmps)) throw cseis_geolib::csException("must enter ncdps");
	...
	do {
		...
		noff++;
		if(noff>mxfold) throw cseis_geolib::csException("cvstacks: input cdp has more traces than mxfold");
		if(ieod) {
			iend++;
		}else{
