...
void* main_suahw( void* args )
{
	...
	/* Get parameters */
	if (!parObj.getparstring("mode", &mode)) mode = "extrapolate";
	...
	if (!parObj.getparstring("key2", &key2)) key2 = "fldr";
	if (!parObj.getparstring("key3", &key3)) key3 = "tracf";

	/* get a's */
	nvals = parObj.countparval("a");
	if ( nvals==0 ) throw cseis_geolib::csException("need array of a-values!");
	a = ealloc1double(nvals);
	parObj.getpardouble("a",a);

	/* get b's */
	if ( (i=parObj.countparval("b"))!=nvals )
			throw cseis_geolib::csException("number of b's and a's must be equal!");
	b = ealloc1double(nvals);
	parObj.getpardouble("b",b);

	/* are b's in ascending order? */
	for ( i=1; i<nvals; ++i ) {
		if ( b[i-1] >= b[i] ) throw cseis_geolib::csException("b's must be in ascending order!");
	}

	/* get c's */
	if ( (nvalsc=parObj.countparval("c")) > 0 ) {
