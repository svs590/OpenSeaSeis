...
void* main_suea2df( void* args )
{
	...
	if (!parObj.getparfloat("dt",&dt)) dt = 0.001;
	if (!parObj.getparfloat("ft",&ft)) ft = 0.0;
	if (!parObj.getparfloat("lt",&lt)) lt = 1.0;

	/* get model size in slow direction */
	if (!parObj.getparint("nx",&nx)) nx = 200;
	if (!parObj.getparfloat("dx",&dx)) dx = 10;
	if (!parObj.getparfloat("fx",&fx)) fx = -1000;

	/* get model size in fast direction */
	if (!parObj.getparint("nz",&nz)) nz = 100;
	if (!parObj.getparfloat("dz",&dz)) dz = dx;
	if (!parObj.getparfloat("fz",&fz)) fz = 0;
	...
	/* get source location */
	if (!parObj.getparfloat("sx",&sx)) sx = (fx+xmax)/2;
	if (!parObj.getparfloat("sz",&sz)) sz = (fz+zmax)/2;

	/* get source type, angle */	
	if (!parObj.getparstring("stype",&stype)) stype="p";
	if (!parObj.getparfloat("sang",&sang)) sang = 0;

	/* get wavelet type, source time length, average frequency */
	if (!parObj.getparstring("wtype",&wtype)) wtype="ri";
	if (!parObj.getparfloat("ts",&ts)) ts = 0.05;
	if (!parObj.getparfloat("favg",&favg)) favg = 50;

	if (!parObj.getparint("qsw",&qsw)) qsw = 0;
	if (!parObj.getparint("aniso",&aniso)) aniso = 0;
	if (!parObj.getparint("tsw",&tsw)) tsw = 0;
	...
	parObj.getparstring("snfile",&snfile);
	parObj.getparstring("sofile",&sofile);

	/* determine snapshots times and sample numbers */
	nsnap = parObj.countparval("snaptime");
	if (nsnap!=0){
		snaptime = alloc1float(nsnap);
		isnap = alloc1int(nsnap);
		parObj.getparfloat("snaptime",snaptime);
		warn("nsnap=%d \n",nsnap);
	...
	if (*snfile!='\0') {
		if((sneisfp=fopen(snfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open snfile=%s\n",snfile);
	}

	/* if requested, open file for source */
	if (*sofile!='\0') {
		if((soeisfp=fopen(sofile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open sofile=%s\n",sofile);
	}
	...
	if (!parObj.getparstring("c11file",&c11file)) 	c11file="c11_file";
	if (!parObj.getparstring("c55file",&c55file)) 	c55file="c55_file";

	/*.. open file for C11 */
	if((c11filefp=fopen(c11file,"r"))==NULL)
			throw cseis_geolib::csException("cannot open c11file=%s\n",c11file);
	/*.. open file for C55 */
	if((c55filefp=fopen(c55file,"r"))==NULL)
			throw cseis_geolib::csException("cannot open c55file=%s\n",c55file);


	if (aniso==1) { 
		/* Must have these additional stiffnesses for anisotropy */
		if (!parObj.getparstring("c13file",&c13file))	c13file="c13_file";
		if (!parObj.getparstring("c35file",&c35file))	c35file="c35_file";
		if (!parObj.getparstring("c33file",&c33file))	c33file="c33_file";
		if (!parObj.getparstring("c15file",&c15file))	c15file="c15_file";

		/*.. open file for C13 */
		if((c13filefp=fopen(c13file,"r"))==NULL)
				throw cseis_geolib::csException("cannot open c13file=%s\n",c13file);

		/*.. open file for C35 */
		if((c35filefp=fopen(c35file,"r"))==NULL)
				throw cseis_geolib::csException("cannot open c35file=%s\n",c35file);

		/*.. open file for C33 */
		if((c33filefp=fopen(c33file,"r"))==NULL)
				throw cseis_geolib::csException("cannot open c33file=%s\n",c33file);

		/*.. open file for C15 */
		if((c15filefp=fopen(c15file,"r"))==NULL)
				throw cseis_geolib::csException("cannot open c15file=%s\n",c15file);
	}
	/**** end ....... stiffnesses .... */
	...
	if (qsw==1) {
		...
		if (!parObj.getparstring("qfile",&qfile))	qfile="q_file";

		/*.. open file for Q */
		if((qfilefp=fopen(qfile,"r"))==NULL)
				throw cseis_geolib::csException("cannot open qfile=%s\n",qfile);
	}
	/**** end ....... attenuation .... */
	...
	nbc = parObj.countparval("bc");
	if (nbc==4) {
		parObj.getparint("bc", bc);	
	} else {
		bc[0] = 10;
		bc[1] = 10;
		bc[2] = 10;
	...
	if (!parObj.getparfloat("bc_a",&bc_a)) bc_a = 0.95;
	if (!parObj.getparfloat("bc_r",&bc_r)) bc_r = 0;
	warn("%f bc_a  %f bc_r\n",bc_a,bc_r);
	...
	if (!parObj.getparint("verbose",&verbose)) verbose = 0;
	if (!parObj.getparstring("hsfile",&hsfile)) hsfile="hs.su";
	if (!parObj.getparstring("vrslfile",&vrslfile)) vrslfile="vsp.su";
	warn("hs1=%d vs2=%d \n",hs1, vs2);
	...
	if((vseisfp=fopen(vrslfile,"w+"))==NULL)
		throw cseis_geolib::csException("cannot open vrslfile=%s\n",vrslfile);
	...
	/* check to se if source is inside the model */
 	if((sx<fx) || (sx>xmax))
		throw cseis_geolib::csException("sx=%f source must lie within model \n",sx);
 	if((sz<fz) || (sz>zmax))
		throw cseis_geolib::csException("sz=%f source must lie within model \n",sz);
	...
	if((mfp=fopen(mfile,"w+"))==NULL)
		throw cseis_geolib::csException("cannot open mfile=%s\n",mfile);
	...
	if (*rhofile!='\0') {
		...
		if (fread(rhotemp[0],sizeof(float),nx*nz,rhofilefp)!=nx*nz)
			throw cseis_geolib::csException("error reading rhofile=%s\n",rhofile);
		fclose(rhofilefp);
	...
	if (*rhofile=='\0' ) {
		throw cseis_geolib::csException("!!!rhofile= not set!!!");
	...
	if (fread(c11temp[0],sizeof(float),nx*nz,c11filefp)!=nx*nz)
		throw cseis_geolib::csException("error reading c11file=%s\n",c11file);
	fclose(c11filefp);
	...
	if (fread(c55temp[0],sizeof(float),nx*nz,c55filefp)!=nx*nz)
		throw cseis_geolib::csException("error reading c55file=%s\n",c55file);
	fclose(c55filefp);
	...
	if (aniso==1) {  /* if anisotropy */
		...
		if (fread(c13temp[0],sizeof(float),nx*nz,c13filefp)!=nx*nz)
			throw cseis_geolib::csException("error reading c13file=%s\n",c13file);
		fclose(c13filefp);
		...
		if (fread(c33temp[0],sizeof(float),nx*nz,c33filefp)!=nx*nz)
			throw cseis_geolib::csException("error reading c33file=%s\n",c33file);
		fclose(c33filefp);
		...
		if (fread(c15temp[0],sizeof(float),nx*nz,c15filefp)!=nx*nz)
			throw cseis_geolib::csException("error reading c15file=%s\n",c15file);
		fclose(c15filefp);
		...
		if (fread(c35temp[0],sizeof(float),nx*nz,c35filefp)!=nx*nz)
			throw cseis_geolib::csException("error reading c35file=%s\n",c35file);
		fclose(c35filefp);
	...
	if (qsw==1) {  /* if attenuation */
		...
		if (fread(qtemp[0],sizeof(float),nx*nz,qfilefp)!=nx*nz)
			throw cseis_geolib::csException("error reading qfile=%s\n",qfile);
		fclose(qfilefp);
	...
	if((hchdfp=fopen(hcfile,"w+"))==NULL)
		throw cseis_geolib::csException("cannot open hcfile=%s\n",hcfile);
	...
	if((vchdfp=fopen(vrshfile,"w+"))==NULL)
		throw cseis_geolib::csException("cannot open vrshfile=%s\n",vrshfile);
	write_chd(nxpadded,nzpadded,nt,dx,dz,dt,fx,xmax,fz,zmax,
			sx,sz,favg,ts,wtype,stype,
