...
void* main_suimp3d( void* args )
{
	...
	/* Get parameters */
	if (!parObj.getparint("nshot", &nshot))	nshot = 1;
	if (!parObj.getparint("nrec", &nrec))		nrec  = 1;
	if (!parObj.getparint("nt", &nt))		nt    = 256;
	if (!parObj.getparfloat("c", &c))		c     = 5000.0;
	if (!parObj.getparfloat("dt", &dt))		dt    = 0.004;
	if (!parObj.getparfloat("x0", &x0))		x0    = 1000.0;
	if (!parObj.getparfloat("y0", &y0))		y0    = 0.0;
	if (!parObj.getparfloat("z0", &z0))		z0    = 1000.0;
	if (!parObj.getparint("dir", &dir))		dir  = 0;
	if (!parObj.getparfloat("sxmin", &sxmin))	sxmin = 0.0;
	if (!parObj.getparfloat("symin", &symin))	symin = 0.0;
	if (!parObj.getparfloat("szmin", &szmin))	szmin = 0.0;
	if (!parObj.getparfloat("gxmin", &gxmin))	gxmin = 0.0;
	if (!parObj.getparfloat("gymin", &gymin))	gymin = 0.0;
	if (!parObj.getparfloat("gzmin", &gzmin))	gzmin = 0.0;
	if (!parObj.getparfloat("dsx", &dsx))		dsx   = 100.0;
	if (!parObj.getparfloat("dsy", &dsy))		dsy   = 0.0;
	if (!parObj.getparfloat("dsz", &dsz))		dsz   = 0.0;
	if (!parObj.getparfloat("dgx", &dgx))		dgx   = 100.0;
	if (!parObj.getparfloat("dgy", &dgy))		dgy   = 0.0;
	if (!parObj.getparfloat("dgz", &dgz))		dgz   = 0.0;
