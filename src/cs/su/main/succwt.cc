...
void* main_succwt( void* args )
{
	...
	/* Get parameters */
	if (!parObj.getparint	("noct", &noct))	noct = 5;
	if (!parObj.getparint	("nv", &nv))		nv = 10;
	if (!parObj.getparint	("verbose", &verbose))	verbose = 0;
	if (!parObj.getparint	("out", &out))	 	out = 1;
	if (!parObj.getparint	("fs", &fs))		fs = 1;
	if (!parObj.getparfloat ("p", &p))		p = -0.5;
	...
	if (!parObj.getparfloat ("fmax", &fmax)) fmax = 1/(2*dt);
	if (!parObj.getparfloat ("c", &c)) c = 1/(2*fmax);
		warn("c = %g",c);

	/* avoid nf=nt in linear sampling */
	if (!parObj.getparfloat ("df", &df)) df = 1.0;
