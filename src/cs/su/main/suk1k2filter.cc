...
void* main_suk1k2filter( void* args )
{
	...
	su2cs->setSUDoc( sdoc_suk1k2filter );
	...
	nK2 = nx2fft/2 + 1;
        ...
        if ((npoly1 = parObj.countparval("k1"))!=0) {
                k1 = ealloc1float(npoly1);
                parObj.getparfloat("k1",k1);
        } else {
                npoly1 = 4;
                k1 = ealloc1float(npoly1);
	...
	/* Get k1 filter amplitude values */
        if ((namps1 = parObj.countparval("amps1"))!=0) {
                amps1 = ealloc1float(namps1);
                parObj.getparfloat("amps1",amps1);
        } else {
                namps1 = npoly1;
                amps1 = ealloc1float(namps1);
	...
	if (!(namps1==npoly1)) 
		throw cseis_geolib::csException("number of k1 values must = number of amps1 values");

        /* Check amps1 values */
        for(iamps = 0, icount=0; iamps < namps1 ; iamps++) {
		icount+=amps1[iamps];
                if( amps1[iamps] < 0.) throw cseis_geolib::csException("amps1 values must be positive");
        }
        if (icount==0) throw cseis_geolib::csException("All amps1 values are zero");
        ...
        if ((npoly2 = parObj.countparval("k2"))!=0) {
                k2 = ealloc1float(npoly2);
                parObj.getparfloat("k2",k2);
        } else {
                npoly2 = 4;
                k2 = ealloc1float(npoly2);
	...
	/* Get k2 filter amplitude values */
        if ((namps2 = parObj.countparval("amps2"))!=0) {
                amps2 = ealloc1float(namps2);
                parObj.getparfloat("amps2",amps2);
        } else {
                namps2 = npoly2;
                amps2 = ealloc1float(namps2);
	...
	if (!(namps2==npoly2)) 
		throw cseis_geolib::csException("number of k2 values must = number of amps2 values");
        
	
        /* Check amps2 values */
        for(iamps = 0, icount=0; iamps < namps2 ; ++iamps) {
		icount+=amps2[iamps];
                if( amps2[iamps] < 0.) throw cseis_geolib::csException("amps2 values must be positive");
        }
        if (icount==0) throw cseis_geolib::csException("All amps2 values are zero");
        ...
        if (!parObj.getparint("quad",&quad))		quad=0;
