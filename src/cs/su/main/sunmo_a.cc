...
void* main_sunmo_a( void* args )
{
	...
	ncdp = parObj.countparval("cdp");
	if (ncdp>0) {
		if (parObj.countparname("vnmo")!=ncdp)
			throw cseis_geolib::csException("a vnmo array must be specified for each cdp");
		if (parObj.countparname("tnmo")!=ncdp)
			throw cseis_geolib::csException("a tnmo array must be specified for each cdp");
		if (parObj.countparname("anis1")!=ncdp &&
		    parObj.countparname("anis1")!=0)
			throw cseis_geolib::csException("an anis1 array must be specified for each cdp, "
			    "or omitted at all");
		if (parObj.countparname("anis2")!=ncdp &&
		    parObj.countparname("anis2")!=0)
			throw cseis_geolib::csException("an anis2 array must be specified for each cdp, "
			    "or omitted at all");
	} else {
		ncdp = 1;
		if (parObj.countparname("vnmo")>1)
			throw cseis_geolib::csException("only one (or no) vnmo array must be specified");
		if (parObj.countparname("tnmo")>1)
			throw cseis_geolib::csException("only one (or no) tnmo array must be specified");
		if (parObj.countparname("anis1")>1)
			throw cseis_geolib::csException("only one (or no) anis1 array must be specified");
		if (parObj.countparname("anis2")>1)
			throw cseis_geolib::csException("only one (or no) anis2 array must be specified");
	}
	cdp = ealloc1float(ncdp);
	if (!parObj.getparfloat("cdp",cdp)) cdp[0] = tr.cdp;
	ovv = ealloc2float(nt,ncdp);
	oa1 = ealloc2float(nt,ncdp);
	oa2 = ealloc2float(nt,ncdp);
	for (icdp=0; icdp<ncdp; ++icdp) {

		nvnmo = parObj.countnparval(icdp+1,"vnmo");
		ntnmo = parObj.countnparval(icdp+1,"tnmo");
		nanis1 = parObj.countnparval(icdp+1,"anis1");
		nanis2 = parObj.countnparval(icdp+1,"anis2");

		if (nvnmo!=ntnmo && !(ncdp==1 && nvnmo==1 && ntnmo==0))
			throw cseis_geolib::csException("number of vnmo and tnmo values must be equal");
		if (nanis1!=nvnmo && nanis1 != 0)
			throw cseis_geolib::csException("number of vnmo and anis1 values must be equal");
		if (nanis2!=nvnmo && nanis2 != 0)
			throw cseis_geolib::csException("number of vnmo and anis2 values must be equal");
		...
		if (!parObj.getnparfloat(icdp+1,"vnmo",vnmo)) vnmo[0] = 1500.0;
		if (!parObj.getnparfloat(icdp+1,"tnmo",tnmo)) tnmo[0] = 0.0;
		if (!parObj.getnparfloat(icdp+1,"anis1",anis1)) 
			for (i=0; i<nvnmo; i++) anis1[i] = 0.0;
		if (!parObj.getnparfloat(icdp+1,"anis2",anis2))
			for (i=0; i<nvnmo; i++) anis2[i] = 0.0;
		for (it=1; it<ntnmo; ++it)
			if (tnmo[it]<=tnmo[it-1])
				throw cseis_geolib::csException("tnmo values must increase monotonically");
	...
	if (smute<=0.0) throw cseis_geolib::csException("smute must be greater than 0.0");
	if (!parObj.getparint("lmute",&lmute)) lmute = 25;
	if (!parObj.getparint("sscale",&sscale)) sscale = 1;
	if (!parObj.getparint("invert",&invert)) invert = 0;
	if (!parObj.getparint("upward",&upward)) upward = 0;
        parObj.checkpars();
	...
	do {
		...
		if (newsloth || tr.offset!=oldoffset) {
			...
			for (it=0,tn=ft/dt; it<nt; ++it,tn+=1.0) {
				...
				if (tsq<0.0)
					throw cseis_geolib::csException("negative moveout; check anis1, "
					    "anis2, or suwind far-offset "
					    "traces");
				if ((1.0+oa2t[it]*temp)<=0.0)
					throw cseis_geolib::csException("anis2 negative and too small; "
