...
void* main_suop2( void* args )
{
	...
	parObj.getparfloat( "w1" ,&w1 );
	parObj.getparfloat( "w2" ,&w2 );
	...
	if	(STREQ(op, "sum"))	iop = ADD;
		...
		throw cseis_geolib::csException("unknown operation=\"%s\", see self-doc", op);

	/* parObj.getpar trid if set */
	if ( iop == ZIPPER ) {
		/* Get optional parameter trid */
		if (!parObj.getparint( "trid" ,&trid ))	trid = FUNPACKNYQ;
	...
	if (iop <= TPANOP) { /* do panel op panel operations */
		...
		while (fgettr(fp1, &intrace1) &&
			...
			if ((nt = intrace1.ns) != intrace2.ns) {
				warn("trace %d:", itr);
				throw cseis_geolib::csException("%s and %s have different ns (%d vs %d)",
