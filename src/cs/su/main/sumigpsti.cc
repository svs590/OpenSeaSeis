...
void* main_sumigpsti( void* args )
{
	...
	/* get parameters */
	if (!(nfil=parObj.getparfloat("ffil",ffil))) {
		ffil[0] = ffil[1] = 0.0;
		ffil[2] = ffil[3] = 0.5/dt;
	} else if (nfil!=4) {
		throw cseis_geolib::csException("if ffil specified, exactly 4 values must be provided");
	}
	if (!parObj.getparint("nxpad",&nxpad)) nxpad=0;
	if (!parObj.getparint("ltaper",&ltaper)) ltaper=0;
	if (!parObj.getparint("verbose",&verbose)) verbose=0;
	if (!parObj.getparint("np",&np)) np=200;
	if (!parObj.getparint("ntflag",&ntflag)) ntflag=1;

	/* determine NMO velocity function vnm(t) */
	vnt = ealloc1float(nt);
        if (!parObj.getparstring("vnfile",&vnfile)) {
                ntmig = parObj.countparval("tmig");
                if (ntmig==0) ntmig = 1;
                tmig = ealloc1float(ntmig);
                if (!parObj.getparfloat("tmig",tmig)) tmig[0] = 0.0;
                nvnmig = parObj.countparval("vnmig");
                if (nvnmig==0) nvnmig = 1;
                if (nvnmig!=ntmig) throw cseis_geolib::csException("number of tmig and vnmig must be equal");
                vnmig = ealloc1float(nvnmig);
                if (!parObj.getparfloat("vnmig",vnmig)) vnmig[0] = 1500.0;
                for (itmig=1; itmig<ntmig; ++itmig)
                        if (tmig[itmig]<=tmig[itmig-1])
                                throw cseis_geolib::csException("tmig must increase monotonically");
                for (it=0,t=0.0; it<nt; ++it,t+=dt)
                        intlin(ntmig,tmig,vnmig,vnmig[0],vnmig[ntmig-1],
                                1,&t,&vnt[it]);
        } else {
                if (fread(vnt,sizeof(float),nt,fopen(vnfile,"r"))!=nt)
                        throw cseis_geolib::csException("cannot read %d velocities from file %s",nt,vnfile);
        }

	/* determine function eta(t) */
	etat = ealloc1float(nt);
        if (!parObj.getparstring("etafile",&etafile)) {
                ntmig = parObj.countparval("tmig");
                if (ntmig==0) ntmig = 1;
                tmig = ealloc1float(ntmig);
                if (!parObj.getparfloat("tmig",tmig)) tmig[0] = 0.0;
                netamig = parObj.countparval("etamig");
                if (netamig==0) netamig = 1;
                if (netamig!=ntmig) throw cseis_geolib::csException("number of tmig and etamig must be equal");
                etamig = ealloc1float(netamig);
                if (!parObj.getparfloat("etamig",etamig)) etamig[0] = 0.0;
                for (itmig=1; itmig<ntmig; ++itmig)
                        if (tmig[itmig]<=tmig[itmig-1])
                                throw cseis_geolib::csException("tmig must increase monotonically");
                for (it=0,t=0.0; it<nt; ++it,t+=dt)
                ...
                if (fread(etat,sizeof(float),nt,fopen(etafile,"r"))!=nt)
                        throw cseis_geolib::csException("cannot read %d velocities from file %s",nt,etafile);
        }
	...
	if (!parObj.getparstring("vfile",&vfile)) {
		...
		tmig = ealloc1float(ntmig);
		if (!parObj.getparfloat("tmig",tmig)) tmig[0] = 0.0;
		nvmig = parObj.countparval("vmig");
		if (nvmig!=ntmig) {
			for(it=0; it<nt; ++it)
				vt[it] = vnt[it];
		} else {
			vmig = ealloc1float(nvmig);
			if (!parObj.getparfloat("vmig",vmig)) vmig[0] = vnt[0];
			for (itmig=1; itmig<ntmig; ++itmig)
				if (tmig[itmig]<=tmig[itmig-1])
					throw cseis_geolib::csException("tmig must increase monotonically");
			for (it=0,t=0.0; it<nt; ++it,t+=dt)
