...
void* main_supermute( void* args )
{
	...
	if (!parObj.getparint("check", &check))	check=0;

	/* Get info from first trace */ 
	if (!cs2su->getTrace(&tr))  throw cseis_geolib::csException("cannot get first trace");

	if (!parObj.getparint("n1", &n[1]))	n[1] = (int) tr.ns;
	if (!parObj.getparint("n2", &n[2]))	n[2] = (int) tr.ntr;
	if (!parObj.getparint("n3", &n[3]))	n[3] = 1;
	for(i=1; i<=3; i++)
	{  if (! (n[i])) throw cseis_geolib::csException("invalid dimensions: %d %d %d", n[1], n[2], n[3]);
	}
	if (verbose)
	{  warn("old dimensions: %d %d %d", n[1], n[2], n[3]);
	}

	if (!parObj.getparint("o1", &o[1]))	o[1] = 1;
	if (!parObj.getparint("o2", &o[2]))	o[2] = 2;
	if (!parObj.getparint("o3", &o[3]))	o[3] = 3;
	for(i=1; i<=3; i++)
	{  if (o[i] < 1 || o[i] > 3) throw cseis_geolib::csException("invalid permutation %d: %d", i, o[i]);
	}
	if (verbose)
	...
	if (!parObj.getparint("d1", &d[1]))	d[1] = 1;
	if (!parObj.getparint("d2", &d[2]))	d[2] = 1;
	if (!parObj.getparint("d3", &d[3]))	d[3] = 1;
        parObj.checkpars();

	for(i=1; i<=3; i++)
	{  if (!(d[i])) throw cseis_geolib::csException("invalid interval %d: %d", i, d[i]);
	}
	if (verbose)
	...
	if (! data) throw cseis_geolib::csException("not enough memory: %d %d %d", m[1], m[2], m[3]);
	...
	{  for(j=1; j<=n[2]; j++)
	   {  if (!((k==1) && (j==1)))
		 ...
		 if (!(cs2su->getTrace(&tr)))
	         {  throw cseis_geolib::csException("wrong amount of data: %d, %d (%d), %d (%d)", 
		 	n[1], n[2], j, n[3], k);
	         }
	      }
		 ...
		 {  if ((p < 1) || (p > m[1]) || 
		 	(q < 1) || (q > m[2]) ||
			(r < 1) || (r > m[3]))
	   	    {  throw cseis_geolib::csException("access error: %d (%d) %d, %d (%d) %d, %d (%d) %d",
