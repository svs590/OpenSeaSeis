...
void* main_suwellrf( void* args )
{
	...
	CSMUSTGETPARINT("nval",&nval);
	parObj.getparstring("dfile",&dfile);
	parObj.getparstring("vfile",&vfile);
	parObj.getparstring("rhofile",&rhofile);
	parObj.getparstring("dvfile",&dvfile);
	parObj.getparstring("drfile",&drfile);
	parObj.getparstring("dvrfile",&dvrfile);

	if (!parObj.getparint("ntr",&ntr)) ntr=1;;
	...
	if ( (*dvrfile!='\0')) {
		...
		if((dvrfp=efopen(dvrfile,"r"))==NULL)
                        throw cseis_geolib::csException("cannot open dvrfile=%s",dvrfile); 
		...
		if((dvfp=efopen(dvfile,"r"))==NULL)
                        throw cseis_geolib::csException("cannot open dvfile=%s\n",dvfile); 

		if((drfp=efopen(drfile,"r"))==NULL)
                        throw cseis_geolib::csException("cannot open drfile=%s\n",drfile); 
		...
		if((vfp=efopen(vfile,"r"))==NULL)
                        throw cseis_geolib::csException("cannot open vfile=%s\n",vfile); 

		if((rfp=efopen(rhofile,"r"))==NULL)
                        throw cseis_geolib::csException("cannot open rhofile=%s\n",rhofile); 
	...
	for (i=0; i<nval; ++i) {
		...
		if (i>0) {
			if ((tempd[i] - tempd[i-1]) < 0)
				throw cseis_geolib::csException("depth not monotonically increasing!");
	...
	/* Set output dt, ns */
	if (!parObj.getparfloat("dtout", &dtout))            dtout = dtmin/2.0;
	...
	/* Output traces */
	for(i=0; i<ntr; ++i) su2cs->putTrace(&tr);
