...
void* main_sufdmod2( void* args )
{
	...
	if (!parObj.getparfloat("tmax",&tmax)) throw cseis_geolib::csException("must specify tmax!\n");
	...
	/*--------------------------------------------------------------------*\
	  ...
	  itself.
	\*--------------------------------------------------------------------*/
	parObj.getparstring("xs",&spfile); /* See if the user specified a source
                                       point file name. */
	srpntfp = fopen (spfile, "r");
	if ( srpntfp != (FILE *)NULL ) {
		if ( (is = fscanf (srpntfp, "%d", &nxs)) != 1 ) {
			fclose (srpntfp);
			throw cseis_geolib::csException("error reading nxs from spfile=%s\n", spfile);
		}
		nzs = nxs;
	} else {
		nxs = parObj.countparval("xs");
		nzs = parObj.countparval("zs");
	}
	if (nxs!=nzs)
		throw cseis_geolib::csException("number of xs = %d must equal number of zs = %d\n",
			nxs,nzs);
	ns = nxs;

	/* plane wave src */
	if (!parObj.getparint("pw",&pw)) pw=0;
	if (pw==1) ns = nx;

	if (ns==0) throw cseis_geolib::csException("must specify xs and zs!\n");
	xs = alloc1float(ns);
	zs = alloc1float(ns);
	ixs = alloc1int(ns);
	izs = alloc1int(ns);
	...
	if ( srpntfp != (FILE *)NULL ) {
		...
		if ((is = fscanf (srpntfp, "%f %f", &xs[it], &zs[it])) != 2 ) {
			fclose (srpntfp);
	      		throw cseis_geolib::csException("error reading xs[%d], zs[%d] from spfile=%s\n",it,it,spfile);
		}
	}
		fclose (srpntfp);
	} else {
	  parObj.getparfloat("xs",xs);
	  parObj.getparfloat("zs",zs);
	}
	if (!parObj.getparfloat("sstrength",&sstrength))	sstrength =1.0;

	if (pw==1) ztmp = zs[0];
	
	nabs = parObj.countparval("abs");
	if (nabs==4) {
		parObj.getparint("abs", abs);	
	} else {
		abs[0] = ABS0;
		abs[1] = ABS1;
		abs[2] = ABS2;
	...
	if (!parObj.getparint("nt",&nt)) nt = 0;
	...
	if (!parObj.getparint("pwt",&pwt)) pwt = 20;
	if (!parObj.getparint("mono",&mono)) mono = 0;
	...
	if (!parObj.getparfloat("hsz",&hsz)) hsz = 0.0;
	hs1 = NINT( (hsz - fz)/dz );

	if (!parObj.getparfloat("vsx",&vsx)) vsx = 0.0;
	vs2 = NINT((vsx - fx)/dx );
	
	if (!parObj.getparint("verbose",&verbose)) verbose = 0;
	parObj.getparstring("dfile",&dfile);
	parObj.getparstring("hsfile",&hsfile);
	parObj.getparstring("vsfile",&vsfile);
	parObj.getparstring("ssfile",&ssfile);
	...
	if (!parObj.getparfloat("fmax", &fmax))	fmax = vmin/(10.0*h);

	/* compute or set peak frequency for ricker wavelet */
	if (!parObj.getparfloat("fpeak", &fpeak))	fpeak = 0.5*fmax;
	...
	if (*hsfile!='\0') {
		if((hseisfp=fopen(hsfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open hsfile=%s\n",hsfile);
	...
	if (*vsfile!='\0') {
		if((vseisfp=fopen(vsfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open vsfile=%s\n",vsfile);
	...
	if (*ssfile!='\0') {
		if((sseisfp=fopen(ssfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open ssfile=%s\n",ssfile);
	...
	if (*dfile!='\0') {
		...
		if (fread(od[0],sizeof(float),nx*nz,densityfp)!=nx*nz)
			throw cseis_geolib::csException("error reading dfile=%s\n",dfile);
		fclose(densityfp);
		dmin = dmax = od[0][0];
		for (ix=0; ix<nx; ++ix) {
			for (iz=0; iz<nz; ++iz) {
