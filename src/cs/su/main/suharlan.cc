...
void* main_suharlan( void* args )
{
	...
	if (!cs2su->getTrace(&tro1))  throw cseis_geolib::csException("can't get first trace");
	nt = tro1.ns;
	dt = (float) tro1.dt/1000000.0;

/* get general flags and set their defaults */
	if (!parObj.getparint("anenv",&anenv))			anenv	= 1;
	if (!parObj.getparint("scl",&scl))			scl	= 0;
	if (!parObj.getparint("plot",&plot))			plot	= 3;
	if (!parObj.getparint("norm",&norm))			norm	= 1;
	if (!parObj.getparint("verbose",&verbose))		verbose	= 1;
	if (!parObj.getparint("rgt",&rgt))			rgt	= 2;
	if (!parObj.getparint("sts",&sts))			sts	= 1;
	if (!parObj.getparint("gopt",&gopt))			gopt	= 1;

	/* get general parameters and set their defaults */
	if (!parObj.getparfloat("dx",&dx)) 			dx	= 20.0;
	if (!parObj.getparfloat("fx",&fx)) 			fx	= 0.0;
	if (!parObj.getparint("nintlh",&nintlh))		nintlh	= 50;
	if (!parObj.getparint("sditer",&sditer))		sditer	= 5;
	if (!parObj.getparfloat("c",&c)) 			c	= 0.05;
	if (!parObj.getparfloat("rel1",&rel1)) 		rel1	= 0.5;
	if (!parObj.getparfloat("rel2",&rel2)) 		rel2	= 0.75;
	if (!parObj.getparint("niter",&niter))			niter	= 1;
	if (!parObj.getparfloat("dt",&dt))			dt	= 0.004;

	/* get smoothing parameters and set their defaults */
	if (!parObj.getparfloat("r1",&r1))			r1	= 10.;
	if (!parObj.getparfloat("r2",&r2))			r2	= 2.;

	/* get suradon taup parameters and set their defaults */
	if (!parObj.getparfloat("pmin1",&pmin1))		pmin1	= -400.0;
	if (!parObj.getparfloat("pmax1",&pmax1))		pmax1	= 400.0;
	if (!parObj.getparfloat("pmin2",&pmin2))		pmin2	= pmin1;
	if (!parObj.getparfloat("pmax2",&pmax2))		pmax2	= pmax1;
	if (!parObj.getparint("np",&np)) 			np	= 100;
	if (!parObj.getparfloat("prewhite",&prewhite))		prewhite= 0.01;
	if (!parObj.getparfloat("offref",&offref))		offref	= 2000.;
	if (!parObj.getparfloat("interoff",&interoff))		interoff= 0.;
	if (!parObj.getparfloat("depthref",&depthref))		depthref= 0.;
	if (!parObj.getparfloat("pmula",&pmula))		pmula	= pmax1;
	if (!parObj.getparfloat("pmulb",&pmulb))		pmulb	= pmax1;
		if (!parObj.getparint("ninterp",&ninterp))	ninterp	= 0;

	/* get names of output files */
	if (!parObj.getparstring("signal",&signalfile))    signalfile="out_signal";
	if (!parObj.getparstring("noise",&noisefile))	    noisefile="out_noise";

	/* get random generator seed */
	if (!parObj.getparint("seed", &seed)) { /* if not supplied, use clock */
	...
	if (*signalfile !='\0') {
		if ((signal_file=fopen(signalfile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open signal file=%s\n",signalfile);
		erewind(headerfp);
		{	register int itr;
	...
	if (*noisefile!='\0') {
		if ((noise_file=efopen(noisefile,"w"))==NULL)
			throw cseis_geolib::csException("cannot open noise file=%s\n",noisefile);
		erewind(headerfp);
		{	register int itr;
			for (itr=0; itr<ntr; itr++) {
...
******************************************************************************/
{
	...
	if (plot<0||plot>3) throw cseis_geolib::csException("plot flag has to be 0<=plot<=3");
