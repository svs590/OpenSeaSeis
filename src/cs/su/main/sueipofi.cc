...
void* main_sueipofi( void* args )
{
    ...
    if (!parObj.getparstring("interp", &interp)) interp = "cubic";
    if (!parObj.getparstring("file", &file)) file = "polar";
    if (!parObj.getparint("rl1", &rl1)) rl1 = 0;
    if (!parObj.getparint("rl2", &rl2)) rl2 = 0;
    if (!parObj.getparint("pln", &pln)) pln = 0;    
    ...
    else throw cseis_geolib::csException("unknown interp=%s", interp);
    ...
    /* data validation */
    if (iwl<3) throw cseis_geolib::csException("wl=%g must be >= %g sec. (3 samples)", wl, 3.0*dt);
    if (iwl>nt) throw cseis_geolib::csException("wl=%g too long for trace", wl);
    if (pwr<0.0) throw cseis_geolib::csException("pwr=%g must be positive", pwr);
