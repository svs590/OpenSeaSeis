...
void* main_susynlvcw( void* args )
{
	...
	/* get parameters */
	if (!parObj.getparint("nt",&nt)) nt = 101; CHECK_NT("nt",nt);
	...
	if (!parObj.getparfloat("gamma",&gamma)) gamma = 1.0;
	...
	if (!parObj.getparint("ob",&ob)) ob = 1;
	if (!parObj.getparint("sp",&sp)) sp = 1;
