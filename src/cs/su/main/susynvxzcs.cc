...
void* main_susynvxzcs( void* args )
{
	...
	su2cs->setSUDoc( sdoc_susynvxzcs );
	...
	if (!parObj.getparint("nt",&nt)) nt = 501; CHECK_NT("nt",nt);
	if (!parObj.getparfloat("dt",&dt)) dt = 0.004;
	if (!parObj.getparfloat("ft",&ft)) ft = 0.0;
	if (!parObj.getparfloat("fpeak",&fpeak)) fpeak = 0.2/dt;
	if (!parObj.getparint("nxg",&nxg)) nxg = 101;
	if (!parObj.getparfloat("dxg",&dxg)) dxg = 15;
	if (!parObj.getparfloat("fxg",&fxg)) fxg = 0.0;
	if (!parObj.getparint("nxd",&nxd)) nxd = 5;
	if (!parObj.getparint("nxs",&nxs)) nxs = 1;
	if (!parObj.getparfloat("dxs",&dxs)) dxs = 50;
	...
	if (!parObj.getparint("cable",&cable)) cable = 1;
	...
	for (ixs=0; ixs<nxs; ++ixs) {
		...
		xs = fxs+ixs*dxs;
		if (fx>xs || ex<xs) 
		throw cseis_geolib::csException("shot %i lies outside of specified (x,z) grid\n",ixs);
	}
	...
	for (ixs=0; ixs<2; ++ixs) {
		for (ixg=0; ixg<nxg; ++ixg) {
			...
			if (fx>xg || ex<xg)
			throw cseis_geolib::csException("receiver %i lies outside of specified (x,z) grid\n",ixs);
		}
	}
	...
	if(fread(vold[0],sizeof(float),nx*nz,vfp)!=nx*nz)
		throw cseis_geolib::csException("cannot read %d velocities from file %s",nx*nz,vfp);
	if(pert) {
	...
	for (ixs=0, tracl=0; ixs<nxs; ++ixs){
		...
		throw cseis_geolib::csException("\tEikonal equation fails to solve at "
		    "x=%g, z=%g\n\tfrom shot xs=%g .\n",x_err+xs,z_err,xs);
		}
