...
void* main_sufdmod1( void* args )
{
	...
	if (!parObj.getparfloat("sz", &sz)) throw cseis_geolib::csException("must specify sz ! ");
	...
	if (!parObj.getparint ("styp", &styp)) styp=0;
	if (verbose) warn("styp= %d ", styp);
	if (!parObj.getparfloat ("dz", &dz)) dz=1;
	...
	if (!parObj.getparfloat("rz", &rz)) rz=0.0;
	irz = NINT ((rz-fz)/dz);
	if (verbose) warn("receiver depth : %f on knot # %d ", rz, irz);

	if (!parObj.getparfloat("freq", &freq)) freq=15.0;
	if (verbose) warn("frequency : %f  Hz",freq);

	parObj.getparstring ("velfile", &velfile);
	if (verbose) {
	...
	/* read velocity file */
	if (*velfile != '\0' ) {
		if ((velocityfp=fopen(velfile,"r"))=='\0') throw cseis_geolib::csException("cannot open velfile=%s ",velfile);
	}
	if (efread (rv, sizeof(float), nz, velocityfp)!=nz) 
	   throw cseis_geolib::csException("cannot read %d velocity values ", nz);

	/* read density file  and linearly inderpolate on corrrect location */
	if (*dfile != '\0') {
		if ((densityfp=fopen(dfile,"r"))=='\0') throw cseis_geolib::csException("cannot open dfile=%s ",dfile);
		if (fread(rd,sizeof(float), nz, densityfp)!=nz) throw cseis_geolib::csException("error reading dfile %s",dfile);
		fclose(densityfp);
	}
	...
	if (NINT( (float) nt/((float)td))+1>SU_NFLTS) throw cseis_geolib::csException("too many time steps. Increase td ?");
