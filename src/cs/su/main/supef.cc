...
void* main_supef( void* args )
{
	...
	if (!parObj.getparstring("outpar", &outpar))	outpar = OUTPAR_DEFAULT;
	...
	if (ncdp>0) {
		if ((nminlag = parObj.countparval("minlag"))!=0) {
			if (!(nminlag==ncdp)) 
			   throw cseis_geolib::csException("number of cdp values must equal number of minlag values");
		} else {
			throw cseis_geolib::csException("cdp set, but minlag not parObj.getparred!");
		}

		if ((nmaxlag = parObj.countparval("maxlag"))!=0) {
			if (!(nmaxlag==ncdp)) 
			   throw cseis_geolib::csException("number of cdp values must equal number of maxlag values");
		} else {
			throw cseis_geolib::csException("cdp set, but maxlag not parObj.getparred!");
		}
		...
		/* get parameters */
		parObj.getparfloat("minlag",minlag);
		parObj.getparfloat("maxlag",maxlag);
		parObj.getparfloat("cdp",cdp);

		/* check cdp values */
		for (icdp = 0; icdp < ncdp-1; ++icdp)  {
			if(cdp[icdp] > cdp[icdp+1])
				throw cseis_geolib::csException("cdp values must increase monotonically!");
		}
		...
		for (icdp = 0; icdp < ncdp; ++icdp) {
			...
			if (iminlag[icdp] < 1) 
				throw cseis_geolib::csException("minlag[%d]=%g too small",
					icdp, minlag[icdp]);
			if (imaxlag[icdp] >= nt)
				throw cseis_geolib::csException("maxlag[%d]=%g too large",
					icdp, maxlag[icdp]);
			if (iminlag >= imaxlag)
				throw cseis_geolib::csException("minlag[%d]=%g, maxlag[%d]=%g",
		...
		if (parObj.getparfloat("minlag", minlag)) {
			iminlag[0] = NINT(minlag[0]/dt);
		} else {
			iminlag[0] = 1;
		}

		if (parObj.getparfloat("maxlag", maxlag)) {
	...
	/* Get parameters  */
	if (!parObj.getparint("showwiener",  &showwiener))	showwiener = 0;
	if (!parObj.getparfloat("pnoise",  &pnoise))	pnoise = PNOISE;
	
	/* .. mincorr and maxcorr */
	if (parObj.getparfloat("mincorr", &mincorr))	imincorr = NINT(mincorr/dt);
	else					imincorr = 0;
	if (imincorr < 0) throw cseis_geolib::csException("mincorr=%g too small", mincorr);
	
	if (parObj.getparfloat("maxcorr", &maxcorr))	imaxcorr = NINT(maxcorr/dt);
	else					imaxcorr = nt;
	if (imaxcorr > nt) throw cseis_geolib::csException("maxcorr=%g too large", maxcorr);

	if (imincorr >= imaxcorr)
		throw cseis_geolib::csException("mincorr=%g, maxcorr=%g", mincorr, maxcorr);
