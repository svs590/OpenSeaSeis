...
void* main_sugazmig( void* args )
{
	...
	if (!parObj.getparint("ntau",&ntau)) ntau = nt; CHECK_NT("ntau",ntau);
	if (!parObj.getparfloat("dtau",&dtau)) dtau = dt;
	if (!parObj.getparfloat("ftau",&ftau)) ftau = ft;
	if (!parObj.getparfloat("Q",&Q)) Q = 1.0e6;
	if (!parObj.getparfloat("ceil",&ceil)) ceil = 1.0e6; 
