...
/******************************************************************************/
	...
	su2cs->setSUDoc( sdoc_sutifowler );
	...
	if (!parObj.getparstring("printfile",&printfile)) printfile=NULL;
	if (printfile==NULL) {
		fpl=stderr;
	}else{
	...
	if (!parObj.getparfloat("salias",&salias)) salias=0.8;
 	if(salias>1.0) salias=1.0;
	if (!parObj.getparfloat("dtout",&dtout)) dtout=1.5*dt;
	ntout=1+nt*dt/dtout;
	if (!parObj.getparint("getcvstacks",&getcvstacks)) getcvstacks=0;
	if(getcvstacks) {
		dtout=dt;
		ntout=nt;
	}
	...
	if (!parObj.getparstring("file",&file)) file="sutifowler";
	if (!parObj.getparfloat("dx",&dx)) dx=25.;
	if (!parObj.getparfloat("vmin",&vmin)) vmin=1500.;
	if (!parObj.getparfloat("vmax",&vmax)) vmax=8000.;
	if (!parObj.getparfloat("vminstack",&vminstack)) vminstack=vmin;
	if (!parObj.getparfloat("d",&d)) d=0.0;
	if (!parObj.getparfloat("etamin",&etamin)) etamin=0.0;
	if (!parObj.getparfloat("etamax",&etamax)) etamax=0.5;
	if (!parObj.getparfloat("vsvp",&vsvp)) vsvp=0.5;
	if (!parObj.getparint("neta", &neta)) neta = 1;
	if (fabs(etamax-etamin)<1.0e-7) neta = 1;
	if (neta < 1) neta = 1;
	if (!parObj.getparint("choose", &ichoose)) ichoose = 1;
	if (!parObj.getparint("ncdps", &ncmps)) throw cseis_geolib::csException("sutifowler: must enter ncdps");
	if (!parObj.getparint("nv", &nv)) nv = 75;
	if (!parObj.getparint("nvstack", &nvstack)) nvstack = 180;
	if (!parObj.getparint("ntpad", &ntpad)) ntpad = 0.1*ntout;
	if (!parObj.getparint("nxpad", &nxpad)) nxpad = 0;
	if (!parObj.getparint("lmute", &lmute)) lmute = 24;
	lmute=1 + 0.001*lmute/dtout;
	if (!parObj.getparint("lbtaper", &lbtaper)) lbtaper = 0;
	if (!parObj.getparint("lstaper", &lstaper)) lstaper = 0;
	if (!parObj.getparint("mxfold", &mxfold)) mxfold = 120;
	if (!parObj.getparint("nonhyp",&nonhyp)) nonhyp=1.;
	if (!parObj.getparint("ngroup", &ngroup)) ngroup = 20;
	ndir = parObj.countparname("p");
	if(ndir==0) {
		ndir=-1;
	}else{
		dir = (char **)VNDemalloc(ndir*sizeof(char *),"dir");
		for(k=0;k<ndir;k++) {
			it=parObj.getnparstring(k+1,"p",&dir[k]);
		}
	}
        parObj.checkpars();
...
/******************************************************************************/
	...
	if(ichoose>=3){
		...
		if(nvstack<6) throw cseis_geolib::csException("sutifowler: nvstack must be 6 or more");
		...
		for(icmp=0;icmp<ncmps;icmp++) {
			...
			for(iv=0;iv<nvstack;iv++) {
				...
				if( !cs2su->getTrace(&tr) ) {
					...
					throw cseis_geolib::csException("sutifowler: error reading input cvstacks");
				    }
				}
			}
		}
...
/******************************************************************************/
	...
	do {
	   ...
	   noff++;
	   if(noff>mxfold) throw cseis_geolib::csException("tifowler: input cdp has more traces than mxfold");
