...
void* main_sutxtaper( void* args )
{
 ...
 /*   float *x2;             * float array of x2 values                  */
  ...
  if (!parObj.getparfloat("low", &low)) 	low = 0.;
     if (low > 1.0) throw cseis_geolib::csException("low must be less than 1");
  ...
  if (!parObj.getparstring("key", &key)) 	key = "tr";
  if (!parObj.getparfloat("min", &min)) 	min = 0.;
  if (!parObj.getparfloat("max", &max)) 	max = 0.;
  if (!parObj.getparfloat("dx", &dkey)) 	dkey = 1.;
  ...
  if (!have_ntr) warn ("ntr neither set nor parObj.getparred, \n \
