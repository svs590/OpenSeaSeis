...
void* main_sudipdivcor( void* args )
{
	...
	su2cs->setSUDoc( sdoc_sudipdivcor );
	...
	/* get parameters */
	if (!parObj.getparfloat("dxcdp",&dx)) throw cseis_geolib::csException("dxcdp required");
	if (!parObj.getparint("np",&np)) np=50;
	if (!parObj.getparint("trans",&trans)) trans=0;
	if (!parObj.getparint("norm",&norm)) norm=1;
	if (!parObj.getparint("conv",&conv)) conv=0;
	...
	if (!parObj.getparstring ("vfile",&vfile)){
		...
		nv = parObj.countparval("vmig");

		if (nv==0) nv=1;

		if (nv!=ntau) 
			throw cseis_geolib::csException("number of tmig and vmig must be equal");
		...
		if (fread(vt,sizeof(float),nt,fopen(vfile,"r"))!=nt)
			throw cseis_geolib::csException("Not %d velocities in file %s",nt,vfile);
	}
