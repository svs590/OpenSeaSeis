...
void* main_subackush( void* args )
{
	...
	/* get parameters */
	if (!parObj.getparint("navg",&navg)) 	navg = 101;
	if (!parObj.getparint("gr",&gr)) 	gr   = 0;
	if (!parObj.getparfloat("grs",&grs)) 	grs  = 100.0;
	if (!parObj.getparfloat("grc",&grc)) 	grc  = 10.0;
	if (!parObj.getparfloat("se",&se)) 	se   = 0.209;
	if (!parObj.getparfloat("sd",&sd)) 	sd   = 0.033;
	if (!parObj.getparfloat("sg",&sg)) 	sg   = 0.203;
	if (!parObj.getparint("smode",&smode)) smode = 1;
        parObj.checkpars();
