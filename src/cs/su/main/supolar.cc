...
void* main_supolar( void* args )
{
    ...
    /* get parameters ... */
    if (!parObj.getparstring("file", &file)) file="polar";
    if (!parObj.getparstring("angle", &angle)) angle="rad";
    if (!parObj.getparstring("win", &win)) win="boxcar";
    if (!parObj.getparfloat("wl", &wl)) wl = 0.1;
    if (!parObj.getparfloat("dt", &dt)) dt = ((double) tr.dt)/1000000.0;
    if (!parObj.getparfloat("rlq", &rlq)) rlq = 1.0;
    if (!parObj.getparint("verbose", &verbose)) verbose = 0;
    
    /* ... and output flags */
    if (!parObj.getparint("all", &all))       all = 0;
    if (!parObj.getparint("rl", &rl))          rl = (all) ? all : 1;
    if (!parObj.getparint("dir", &dir))       dir = (all) ? 1 : 1;
    if (!parObj.getparint("theta", &theta)) theta = (all) ? all : 0;
    if (!parObj.getparint("phi", &phi))       phi = (all) ? all : 0;
    if (!parObj.getparint("tau", &tau))       tau = (all) ? 1 : 0;
    if (!parObj.getparint("ellip", &ellip)) ellip = (all) ? 1 : 0;
    if (!parObj.getparint("pln", &pln))       pln = (all) ? 1 : 0;
    if (!parObj.getparint("f1", &f1))          f1 = (all) ? 1 : 0;
    if (!parObj.getparint("l1", &l1))          l1 = (all) ? 1 : 0;
    if (!parObj.getparint("amp", &amp))       amp = (all) ? 1 : 0;
    ...
    else throw cseis_geolib::csException("unknown win=%s", win);
    ...
    else throw cseis_geolib::csException("unknown angle=%s", angle);
    ...
    /* data validation */
    if (iwl<1) throw cseis_geolib::csException("wl=%g must be positive", wl);
    if (iwl>nt) throw cseis_geolib::csException("wl=%g too long for trace", wl);
    if (!strlen(file)) throw cseis_geolib::csException("file= not set and default overridden");
    ...
    } while (cs2su->getTrace(&tr)); /* end loop over traces */
