...
void* main_su2voxet( void* args )
{
	...
	if (!parObj.getparint("maxu", &maxu)) maxu = 1000;
	if (!parObj.getparint("maxv", &maxv)) maxv = 1000;
	if (!parObj.getparint("time", &time)) time = 0;
	if (!parObj.getparint("verbose", &verbose)) verbose = 0;
	if (!parObj.getparint("bendian", &bendian)) bendian = 0;
        if (!parObj.getparstring("u", &u)) u = "cdp";
        if (!parObj.getparstring("v", &v)) v = "ep";
	CSMUSTGETPARSTRING("fname",&fname);
	...
	if (!parObj.getparfloat("datum", &datum)) datum = -tr.delrt;
...
/*		fprintf(stderr," %d %d\n",utmp-1,vtmp-1); */
	...
	z0=datum;
	if (!parObj.getparfloat("dz", &dzf)) dzf=-((double)tr.dt/1000.0);
        parObj.checkpars();
	dz=(double)dzf;
