...
void* main_suinvzco3d( void* args )
{
	...
	su2cs->setSUDoc( sdoc_suinvzco3d );
	...
	nt = tr.ns;
	temp = tr.dt;
	if (!parObj.getparfloat("dt",&dt)) dt = temp*0.000001;
	...
	if (!parObj.getparint("ny",&ny)) throw cseis_geolib::csException("must specify ny!\n");
	...
	if (!parObj.getparfloat("dxm",&dx)) dx = 50.;
	if (!parObj.getparfloat("dy",&dy)) dy=50.;
	if (!parObj.getparfloat("dz",&dz)) dz = 50.;
	if (!parObj.getparint("nxb",&nxb)) nxb = nxm/2;
	if (!parObj.getparint("nxc",&nxc)) nxc = 0;
	if (!parObj.getparint("na",&na)) na=100;
	...
	if (!parObj.getparfloat("fyo",&fyo)) fyo=0.0;
	if (!parObj.getparfloat("dyo",&dyo)) dyo = 15.;
        if (!parObj.getparint("nyo",&nyo)) nyo = 101;
	...
	if(nxm*dxm<nxo*dxo || ny*dy<nyo*dyo) throw cseis_geolib::csException("\t not enough range in input  		for the imaging");			    				
		...
		for (iyo=0,yo=fyo; iyo<nyo; ++iyo, yo+=dyo)
		  for (ixo=0,xo=fxo; ixo<nxo; ++ixo,xo+=dxo)
			...
			*/	
			...
			if(ixm<nxm-1||iym<ny-1) cs2su->getTrace(&tr);
