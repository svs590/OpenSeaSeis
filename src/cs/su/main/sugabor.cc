...
void* main_sugabor( void* args )
{
	...
	if (!parObj.getparfloat("fmin", &fmin))	fmin = FRAC0*nyq;
	if (!parObj.getparfloat("fmax", &fmax))	fmax = FRAC1*nyq;
	if (!parObj.getparfloat("band", &band))	band = FRAC2*nyq;
	if (!parObj.getparfloat("beta", &beta))	beta = BETA;
	if (!parObj.getparfloat("alpha",&alpha))	alpha = beta/(band*band);
	...
	do {
		...
		for (i = 0, fcent=fmin; i < nfilt; ++i, fcent+=band) {
			...
			if (!holder) {
				...
				tr.trid = AMPLITUDE; 
				tr.ntr = ntr*nfilt;
				su2cs->putTrace(&tr); /* output time-freq panels */
			} else {
				/* compute instantaneous amplitude */
				for (j = 0; j < nt ; ++j) 
		...
		if (holder) { /* compute the Holder regularity traces */
			...
			/* the regularity measure. 			   */
					...
					if (holder==1) {
						...
						throw cseis_geolib::csException("holder must = 0,1,2!");
					}
			...
			tr.trid = AMPLITUDE; 
			tr.ntr = ntr*nfilt;
			su2cs->putTrace(&tr); /* output holder regularity traces */
		}
