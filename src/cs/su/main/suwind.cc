...
void* main_suwind( void* args )
{
	...
	if (!parObj.getparlong("min", &min))		min = INT_MIN;
	if (!parObj.getparlong("max", &max))		max = INT_MAX;
	if (!parObj.getparint("j", &j))		j = 1;
	if (!parObj.getparint("s", &s))		s = 0;
	if (!parObj.getparulong("count", &count))	count = ULONG_MAX;
	if (!parObj.getparshort("abs", &ab))		ab = 0;
	if (!parObj.getparshort("verbose", &verbose))	verbose = 0;
        if (!parObj.getparint("skip" ,&skip ))         skip=0;
	if (!parObj.getparint("ordered", &ordered))    ordered = 0;
	...
	/* Evaluate time bounds from parObj.getpars and first header */
        ...
        if( skip ){
           ...
           if( skip > ntr-1 ){
              throw cseis_geolib::csException( "can't skip past last trace" );
           }else{
              itr=skip;
              fgettra(infp, &tr, itr);
           }
	...
	if (seismic) {
		...
		if (dt==0)  throw cseis_geolib::csException("dt not set in header or not parObj.getparred");
		if (!parObj.getparfloat("f1",&f1))	f1 = ((double) tr.delrt)/1000.0;  
		if (!f1) {
			if (verbose)
			warn("delrt not set in header or not parObj.getparred");
		}
	} else {	/* is not seismic */
		...
		if (!parObj.getparfloat("dt",&dt)) {
			if (tr.d1) {
				...
				if (dt==0)
				throw cseis_geolib::csException("d1 and dt not set in header or dt not parObj.getparred");
			}
		}
		if (!parObj.getparfloat("f1",&f1)) f1 = tr.f1;
		if (!f1) {       
			if (verbose)
				warn("f1 not set in header or not parObj.getparred");
		}
    }

	/* Time gating parameters */
	if (parObj.getparint("itmin", &itmin)) {
		tmin = itmin*dt + f1;
	} else if (parObj.getparfloat("tmin", &tmin)) {
		itmin = NINT((tmin - f1) / dt);
	} else {
		itmin = 0;
		tmin = f1;
	}
	if (parObj.getparint("itmax", &itmax)) {
		tmax = itmax*dt + f1;
		nt = itmax - itmin + 1;
	} else if (parObj.getparfloat("tmax", &tmax)) {
		itmax = NINT((tmax - f1) / dt);
		nt = itmax - itmin + 1;
	} else if (parObj.getparint("nt", &nt)) {
		itmax = itmin + nt - 1;
		tmax = itmax*dt + f1;
	} else {
		itmax = ntfirst - 1;
	...
	if ((nbad = parObj.countparval("reject"))) {
		badptr = (Value *) ealloc1(nbad, sizeof(Value));
		cseis_getParVal(&parObj,"reject", type, nbad, badptr);
	}


	 /* Getpar the accept vector */
	if ((ngood = parObj.countparval("accept"))) {
		goodptr = (Value *) ealloc1(ngood, sizeof(Value));
		cseis_getParVal(&parObj,"accept", type, ngood, goodptr);
	}
        parObj.checkpars();
