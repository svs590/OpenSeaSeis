...
void* main_sumax( void* args )
{
   ...
   if (!parObj.getparint("verbose", &verbose))
      verbose = 0;
   if (!parObj.getparstring("output", &output))
      output = "ascii";
   if (!(STREQ(output, "ascii") ||
	 STREQ(output, "binary") || STREQ(output, "segy")
       ))
      throw cseis_geolib::csException("%s unknown output selection", output);

   if (!parObj.getparstring("key1", &key1))
      key1 = "fldr";
   if (!parObj.getparstring("key2", &key2))
      key2 = "ep";

   if (!parObj.getparstring("mode", &mode))
      mode = "maxmin";
   ...
   if (STREQ(mode, "rms") && STREQ(output, "segy")) {
      throw cseis_geolib::csException("%s output incompatible with %s mode", output, mode);
   }
   ...
   if (STREQ(mode, "thd")) {
      if (!parObj.getparfloat("threshamp", &trhldamp))
	 trhldamp = 0.;
      if (!parObj.getparfloat("threshtime", &trhldstrt))
	 trhldstrt = 0.;
