...
void* main_sudmotivz( void* args )
{
	...
	dt = tr.dt/1000000.0;
	ft = tr.delrt/1000.0;
	if (ft!=0.0) throw cseis_geolib::csException("ft=%f --- nonzero ft not yet implemented!");
	offset = tr.offset;
	...
	if (!parObj.getparfloat("speed",&speed)) speed = 1.0;
	...
	vrms = ealloc1float(nttab);
        ...
        if (!parObj.getparstring("vnfile",&vnfile)) {
                ...
                nvndmo = parObj.countparval("vndmo");
                if (nvndmo==0) nvndmo = 1;
                if (nvndmo!=ntdmo) throw cseis_geolib::csException("number of tdmo and vndmo must be equal");
                vndmo = ealloc1float(nvndmo);
                if (!parObj.getparfloat("vndmo",vndmo)) vndmo[0] = 1500.0;
	...
	/* determine eta function eta(t) */
        if (!parObj.getparstring("etafile",&etafile)) {
                ...
                tdmo = ealloc1float(ntdmo);
                if (!parObj.getparfloat("tdmo",tdmo)) tdmo[0] = 0.0;
                netadmo = parObj.countparval("etadmo");
                if (netadmo==0) netadmo = 1;
                if (netadmo!=ntdmo) throw cseis_geolib::csException("number of tdmo and etadmo must be equal");
                etadmo = ealloc1float(netadmo);
                if (!parObj.getparfloat("etadmo",etadmo)) etadmo[0] = 0.0;
                for (itdmo=1; itdmo<ntdmo; ++itdmo)
                        if (tdmo[itdmo]<=tdmo[itdmo-1])
                                throw cseis_geolib::csException("tdmo must increase monotonically");
                for (it=0,t=0.0; it<nt; ++it,t+=dt)
        ...
        if (!parObj.getparstring("vfile",&vfile)) {
			...
			for (it=0; it<nt; ++it)
				vt[it] = vnt[it];
                	...
                	if (!parObj.getparfloat("vdmo",vdmo)) vdmo[0] = vnt[0];
                	for (itdmo=1; itdmo<ntdmo; ++itdmo)
                        	if (tdmo[itdmo]<=tdmo[itdmo-1])
                                	throw cseis_geolib::csException("tdmo must increase monotonically");
                	for (it=0,t=0.0; it<nt; ++it,t+=dt)
...
****************************************************************************/
{
	...
	lt = (nt-1)*dt;
	if (tmin>=lt)
		throw cseis_geolib::csException("tmin=%f >= lt=%f:  no data survive the mute!\n",tmin,lt);
