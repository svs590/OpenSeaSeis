...
void* main_sukdmdcs( void* args )
{
	...
	if (!parObj.getparfloat("dxso",&dxso)) throw cseis_geolib::csException("must specify dxso!\n"); 
        ...
        if (nxgo>1 && dxgo != dxso) throw cseis_geolib::csException("in this implementation, dxso must = dxgo!\n");
	...
	if (!parObj.getparfloat("offmax",&offmax)) offmax = 99999.;
        ...
        if (!parObj.getparint("dc",&dc))   dc = 0;
        if (dc > 1) throw cseis_geolib::csException("dc must be 0 or 1!\n");
...
*****************************************************************************/
{
	...
	if(nxe>=nxso) nxe = nxso-1;
        ...
        if(!dc){
            ...
            if(zgx>=(nzo*dzo)+fzo)
                  throw cseis_geolib::csException("Datuming surface is out of range!\n");
        }
		...
		for(iz=t0; iz<nzt; ++iz){
		    tzt[iz] = ax0*tsum[jx][iz]+ax*tsum[jx+1][iz];
                ...
                if(z<=fzo || z>=fzo+(nzo*dzo))                                                    
                      throw cseis_geolib::csException("Datuming surface is out of traveltime range!\n");    
...
/*                                      */
		...
		jt = (int)at;
                ...
                if(x<=gx){
                  ...
                  if(dc==0 && hp==0.0) throw cseis_geolib::csException("Datum and recording surface overlap!\n");
