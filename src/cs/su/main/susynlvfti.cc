...
void* main_susynlvfti( void* args )
{
	...
	su2cs->setSUDoc( sdoc_susynlvfti );
	...
	/* get parameters */
	if (!parObj.getparint("nt",&nt)) nt = 101;
	if (!parObj.getparfloat("dt",&dt)) dt = 0.04;
	if (!parObj.getparfloat("ft",&ft)) ft = 0.0;
        if (!parObj.getparint("kilounits",&kilounits)) kilounits = 1;
	if ((nxo=parObj.countparval("xo"))!=0) {
		xo = ealloc1float(nxo);
		parObj.getparfloat("xo",xo);
	} else {
		if (!parObj.getparint("nxo",&nxo)) nxo = 1;
		if (!parObj.getparfloat("dxo",&dxo)) dxo = 0.05;
		if (!parObj.getparfloat("fxo",&fxo)) fxo = 0.0;
		xo = ealloc1float(nxo);
		for (ixo=0; ixo<nxo; ++ixo)
			xo[ixo] = fxo+ixo*dxo;
	}
	shots = (parObj.getparint("nxs",&nxs) || 
		parObj.getparfloat("dxs",&dxs) || 
		parObj.getparfloat("fxs",&fxs));
	midpoints = (parObj.getparint("nxm",&nxm) || 
		parObj.getparfloat("dxm",&dxm) || 
		parObj.getparfloat("fxm",&fxm)); 
	if (shots && midpoints)
		throw cseis_geolib::csException("cannot specify both shot and midpoint sampling!\n");
	if (shots) {
		if (!parObj.getparint("nxs",&nxs)) nxs = 101;
		if (!parObj.getparfloat("dxs",&dxs)) dxs = 0.05;
		if (!parObj.getparfloat("fxs",&fxs)) fxs = 0.0;
		nxsm = nxs;
		dxsm = dxs;
	} else {
		midpoints = 1;
		if (!parObj.getparint("nxm",&nxm)) nxm = 101;
		if (!parObj.getparfloat("dxm",&dxm)) dxm = 0.05;
		if (!parObj.getparfloat("fxm",&fxm)) fxm = 0.0;
		nxsm = nxm;
		dxsm = dxm;
	}
	if (!parObj.getparint("nxm",&nxm)) nxm = 101;
	if (!parObj.getparfloat("dxm",&dxm)) dxm = 0.05;
	if (!parObj.getparfloat("fxm",&fxm)) fxm = 0.0;
	if (!parObj.getparfloat("x0",&x0)) x0 = 0.0;
	if (!parObj.getparfloat("z0",&z0)) z0 = 0.0;
	if (!parObj.getparfloat("v00",&v00)) v00 = 2.0;
	if (!parObj.getparfloat("dvdx",&dvdx)) dvdx = 0.0;
	if (!parObj.getparfloat("dvdz",&dvdz)) dvdz = 0.0;
	if (!parObj.getparfloat("fpeak",&fpeak)) fpeak = 0.2/dt;
	if (!parObj.getparint("ls",&ls)) ls = 0;
	if (!parObj.getparint("er",&er)) er = 0;
	if (!parObj.getparint("ob",&ob)) ob = 0;
	if (!parObj.getparfloat("tmin",&tmin)) tmin = 10.0*dt;
	if (!parObj.getparint("ndpfz",&ndpfz)) ndpfz = 5;
	if (!parObj.getparint("smooth",&smooth)) smooth = 0;
	if (!parObj.getparint("verbose",&verbose)) verbose = 1;
	cseis_decodeReflectors(&parObj,&nr,&ar,&nxz,&xr,&zr);
	if (!smooth) breakReflectors(&nr,&ar,&nxz,&xr,&zr);

	/********************************/
	/* get optional parameters relating to anisotropy*/
	if (!parObj.getparfloat("angxs",&angxs))     angxs   = 0.;
	if (!parObj.getparfloat("a",&a))	 	a     = 1.0;
	if (!parObj.getparfloat("f",&f))	 	f     = 0.4;
	if (!parObj.getparfloat("l",&l))		l     = 0.3;
	if (!parObj.getparfloat("delta",&delta))	delta     = 0.0;
	if (!parObj.getparfloat("epsilon",&epsilon))  epsilon     = 0.0;
	/********************************/
	/* for computing incidence and reflection angles and velocities */
	if (!parObj.getparint("ntries",&ntries))	ntries   = 20;
	if (!parObj.getparfloat("epsx",&epsx)) epsx  = .001;
	if (!parObj.getparfloat("epst",&epst)) epst  = .0001;

	if (!parObj.getparint("nitmax",&nitmax))  nitmax  = 12;
	/********************************/
...
*****************************************************************************/
{
	...
	double r,oor,vcpi=0.0,vcps=0.0,p11,so,co=0.0,ci=0.0,cz2,
	...
	if (angxs != 0.) {
		...
		si2  = (sx)*(sx);
		alfa = c2*si2+c1*co2;
		beta = sqrt(pow(w1*si2-w2*co2,2.0)+4*si2*co2*w3);
